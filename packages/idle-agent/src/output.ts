...
function normalizeCodeBlockText(value: string): string {
    const safeValue = escapeTerminalControls(value);
    const text = safeValue.trim().length > 0 ? safeValue : '(empty)';
    return text.replace(/```/g, '``\\`');
}

function normalizeListValue(value: string): string {
    return escapeTerminalControls(value).replace(/\r?\n/g, ' ').trim();
}

function toNonEmptyString(value: unknown): string | undefined {
    return typeof value === 'string' && value.trim().length > 0
        ? escapeTerminalControls(value)
        : undefined;
}
...
export function formatMessageHistory(messages: DecryptedMessage[]): string {
    ...
    const sections = messages.map((msg, index) => {
        ...
        const role = normalizeListValue(content?.role ?? 'unknown');
...
export function formatJson(data: unknown): string {
    return JSON.stringify(data, (key, value) => {
        ...
        if (key === 'encryption' || key === 'dataEncryptionKey' || key === 'sessionKey') return undefined;
