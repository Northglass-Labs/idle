...
describe('formatSessionTable', () => {
    ...
    it('renders terminal control characters as inert text', () => {
        const escapeSequence = '\u001b]52;c;dGVzdA==\u0007';
        const output = formatSessionTable([
            makeSession({ metadata: { summary: escapeSequence, path: `/tmp/${escapeSequence}` } }),
        ]);
...
describe('formatMessageHistory', () => {
    ...
    it('does not emit terminal escape sequences from decrypted message text', () => {
        const output = formatMessageHistory([makeMessage({
            content: {
                role: 'assistant',
                content: { type: 'text', text: 'before\u001b[2Jafter\u0007' },
            },
        })]);

        expect(output).not.toContain('\u001b');
        expect(output).not.toContain('\u0007');
        expect(output).toContain('before\\x1b[2Jafter\\x07');
    });
});
...
describe('formatJson', () => {
    ...
    it('never serializes session or record encryption keys', () => {
        const output = formatJson({
            id: 'session-1',
            sessionKey: new Uint8Array(32).fill(1),
            encryption: { key: new Uint8Array(32).fill(2), variant: 'dataKey' },
            dataEncryptionKey: 'wrapped-key',
        });

        expect(JSON.parse(output)).toEqual({ id: 'session-1' });
        expect(output).not.toContain(Buffer.from(new Uint8Array(32).fill(1)).toString('base64'));
    });
});
