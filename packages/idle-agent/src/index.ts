...
import { getMachineHomeDirectory, resumeSessionOnMachine, spawnSessionOnMachine, stopSessionOnMachine, type SupportedAgent } from './machineRpc';
...
function createClient(session: DecryptedSession, creds: Credentials, config: Config): SessionClient {
    return new SessionClient({
        ...
        initialAgentStateVersion: session.agentStateVersion,
    });
}
...
function resolveSessionMachineId(session: DecryptedSession): string {
    ...
    if (typeof metadata.machineId !== 'string' || metadata.machineId.trim().length === 0) {
        throw new Error(`Session ${session.id} is missing machine metadata and cannot be controlled remotely.`);
    }
...
function ensureMachineCanResume(machine: DecryptedMachine): void {
    const metadata = (machine.metadata ?? {}) as {
        resumeSupport?: {
            ...
            idleAgentAuthenticated?: unknown;
        };
    };
    ...
    if (metadata.resumeSupport?.idleAgentAuthenticated === false) {
        throw new Error('Resume is unavailable on this machine. Run `idle-agent auth login` in that machine environment first.');
    }
...
program
    .name('idle-agent')
    .description('Headless CLI for remotely managing Idle coding-agent sessions')
...
program
    ...
    .action(async (opts: {
        ...
        const directory = await resolveRemotePath(
            opts.path,
            () => getMachineHomeDirectory(config, machine, creds.token),
        );
...
program
    ...
    .action(async (sessionId: string, message: string, opts: { yolo?: boolean; wait?: boolean; json?: boolean }) => {
        ...
        try {
            ...
            const completion = opts.wait
                ? client.sendMessageAndWait(message, permissionMode ? { permissionMode } : undefined)
                : null;
            if (!completion) {
...
program
    .command('stop')
    .description('Stop a session on its original machine')
    .argument('<session-id>', 'Session ID or prefix')
    .action(async (sessionId: string) => {
        ...
        await stopSessionOnMachine(config, machine, creds.token, session.id);
