...
export type SessionFieldDecryptionResult =
    | { success: true; value: Record<string, unknown>; binding: 'bound' | 'legacy' }
    | { success: false };
...
export function decryptSessionField(
    encryption: SessionFieldEncryption,
    sessionId: string,
    field: AuthenticatedSessionFieldName,
    version: number,
    ...
    if (bound.success) {
        return { success: true, value: bound.value, binding: 'bound' };
    }
    if (!options.allowLegacy || isAuthenticatedSessionFieldEnvelopeCandidate(plaintext)) {
        return { success: false };
    }
    return { success: true, value: plaintext, binding: 'legacy' };
}
