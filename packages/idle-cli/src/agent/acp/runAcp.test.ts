...
describe('runAcp', () => {
  ...
  it('passes the configured sandbox policy to the provider launch boundary', async () => {
    const sandboxConfig = {
      enabled: true,
    ...
    mocks.mockReadSettings.mockResolvedValue({ machineId: 'machine-1', sandboxConfig } as any);
    ...
    expect(mocks.backendState.constructorArgs.sandboxConfig).toBe(sandboxConfig);
    await mocks.getKillHandler()!();
    await runPromise;
  });

  it('preserves an explicit per-session sandbox opt-out', async () => {
    const sandboxConfig = {
      enabled: true,
      sessionIsolation: 'workspace',
      customWritePaths: [],
    ...
    expect(mocks.backendState.constructorArgs.sandboxConfig).toBeUndefined();
    await mocks.getKillHandler()!();
    await runPromise;
  });
  ...
  it('wires backend messages through mapper into session envelopes', async () => {
    ...
    mocks.getUserMessageHandler()!({
      ...
      messageIdentity: {
        v: 1,
        sessionId: 'session-1',
        messageId: 'sender-owned-request-1',
      },
    });
    ...
    expect(mocks.mockSession.setActiveRequestId.mock.calls).toEqual([
      ['sender-owned-request-1'],
      [null],
    ]);
  ...
  it('exits when backend reports terminal startup status', async () => {
    const opaqueProviderDetail = 'OPAQUE_PROVIDER_STATUS_DETAIL_a84c';
    mocks.backendState.startSessionMessages = [
      { type: 'status', status: 'error', detail: opaqueProviderDetail },
    ];
    ...
    expect(consoleLines()).toContain(`Status: error: ${opaqueProviderDetail}`);
    expect(JSON.stringify(mocks.mockLoggerDebug.mock.calls)).not.toContain(opaqueProviderDetail);
    expect(mocks.mockLoggerDebug).toHaveBeenCalledWith(
      '[ACP] Terminal provider status received; stopping ACP runner',
      { status: 'error', hasDetail: true },
    );
    expect(mocks.mockSession.close).toHaveBeenCalled();
  ...
  it('ignores ACP model and permission mode requests when values do not match advertised options', async () => {
    const opaqueMode = 'OPAQUE_ACP_MODE_f87e';
    const opaqueModel = 'OPAQUE_ACP_MODEL_91ac';
    mocks.backendState.startSessionMessages = [
      {
        type: 'event',
        name: 'config_options_update',
    ...
    expect(mocks.backendState.setModeCalls).toEqual([]);
    expect(mocks.backendState.setModelCalls).toEqual([]);
    const debugOutput = JSON.stringify(mocks.mockLoggerDebug.mock.calls);
    expect(debugOutput).not.toContain(opaqueMode);
