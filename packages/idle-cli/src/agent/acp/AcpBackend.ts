...
import { createIsolatedGeminiRuntimeHome } from '@/gemini/utils/isolatedRuntimeHome';
...
function summarizeSessionMetadataPayload(payload: unknown): string {
  ...
  const configOptions = boundedDiagnosticCount(Array.isArray(asRecord.configOptions) ? asRecord.configOptions.length : 0);
  const modes = asRecord.modes && typeof asRecord.modes === 'object'
    ? (Array.isArray((asRecord.modes as { availableModes?: unknown }).availableModes)
        ? boundedDiagnosticCount((asRecord.modes as { availableModes: unknown[] }).availableModes.length)
        : 0)
    : 0;
  const models = asRecord.models && typeof asRecord.models === 'object'
    ? (Array.isArray((asRecord.models as { availableModels?: unknown }).availableModels)
        ? boundedDiagnosticCount((asRecord.models as { availableModels: unknown[] }).availableModels.length)
        : 0)
    : 0;
...
export interface AcpBackendOptions {
  ...
  /** Log metadata-only session diagnostics to console */
  verbose?: boolean;

  /** Required OS sandbox policy for the provider process. */
...
function nodeToWebStreams(
    ...
    write(chunk) {
      return new Promise((resolve, reject) => {
        const ok = stdin.write(chunk, (err) => {
          if (err) {
            logger.debug('[AcpBackend] Provider stdin write failed');
            reject(err);
          }
        });
        if (ok) {
  ...
  const readable = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      stdout.on('error', (err) => {
        logger.debug('[AcpBackend] Provider stdout stream failed');
...
function normalizeProviderError(error: unknown): Error {
  ...
  if (error && typeof error === 'object') {
    ...
    if (typeof record.message === 'string' && record.message.trim()) {
      const normalized = new Error(record.message, { cause: error });
      if (typeof record.code === 'string' || typeof record.code === 'number') {
        (normalized as NodeJS.ErrnoException).code = String(record.code);
      }
      return normalized;
    }
  }
  ...
  for (let attempt = 1; attempt <= options.maxAttempts; attempt++) {
    try {
      ...
      if (attempt < options.maxAttempts && shouldRetry) {
        ...
        logger.debug('[AcpBackend] Provider operation retry scheduled', {
...
export class AcpBackend implements AgentBackend {
  ...
  private providerRuntimeCleanup: (() => void | Promise<void>) | null = null;
  private sandboxApplied = false;
  ...
  private emit(msg: AgentMessage): void {
    ...
    for (const listener of this.listeners) {
      try {
        ...
        logger.warn('[AcpBackend] Message handler failed');
      }
    }
  }
  ...
  private async releaseSandbox(): Promise<void> {
    ...
    const providerRuntimeCleanup = this.providerRuntimeCleanup;
    this.providerRuntimeCleanup = null;
    const wasApplied = this.sandboxApplied;
    this.sandboxApplied = false;
    ...
    try {
      ...
      try {
        await providerRuntimeCleanup?.();
      } catch (error) {
        cleanupError ??= error;
      } finally {
        if (wasApplied) {
          this.options.onSandboxApplied?.(false);
        }
      }
    }
  ...
  async startSession(initialPrompt?: string): Promise<StartSessionResult> {
    ...
    try {
      ...
      let spawnArgs = args;
      const sandboxRequired = this.options.sandboxConfig?.enabled === true;
      const environmentProfile = this.options.agentName.toLowerCase() === 'gemini' ? 'gemini' : 'acp';
      let childEnvironment = { ...process.env, ...this.options.env };

      if (sandboxRequired) {
        try {
          if (process.platform === 'win32') {
            throw new Error(
              'OS sandboxing is not supported on Windows. Use --no-sandbox explicitly to run this provider without containment.',
            );
          }

          let effectiveSandboxConfig = this.options.sandboxConfig!;
          let explicitEnvironment = this.options.env;
          if (environmentProfile === 'gemini') {
            const runtimeHome = createIsolatedGeminiRuntimeHome();
            this.providerRuntimeCleanup = runtimeHome.cleanup;
            effectiveSandboxConfig = {
              ...effectiveSandboxConfig,
              extraWritePaths: [
                ...effectiveSandboxConfig.extraWritePaths,
                runtimeHome.path,
              ],
              denyReadPaths: [
                ...effectiveSandboxConfig.denyReadPaths,
                ...runtimeHome.sensitiveSourcePaths,
              ],
            };
            explicitEnvironment = {
              ...explicitEnvironment,
              GEMINI_CLI_HOME: runtimeHome.path,
            };
          }

          childEnvironment = buildAgentChildEnvironment(
            environmentProfile,
            process.env,
            explicitEnvironment,
          );
          this.sandboxCleanup = await initializeSandbox(effectiveSandboxConfig, this.options.cwd);
          const wrapSpawn = await prepareSandboxedSpawn();
          const wrapped = wrapSpawn(this.options.command, args);
          ...
          throw new Error(`${this.options.agentName} sandbox initialization failed: ${reason}`, { cause: error });
        }
      } else {
        this.options.onSandboxApplied?.(false);
      }

      // cross-spawn resolves Windows command shims while preserving every
      // provider argument as data instead of joining argv into a shell string.
      this.process = crossSpawn(spawnCommand, spawnArgs, {
      ...
      if (sandboxRequired) {
        this.sandboxApplied = true;
        this.options.onSandboxApplied?.(true);
      }
      ...
      this.process.stderr.on('data', (data: Buffer) => {
        ...
        logger.debug('[AcpBackend] Agent stderr received', {
          bytes: boundedDiagnosticCount(Buffer.byteLength(text, 'utf8')),
          duringInvestigation: hasActiveInvestigation,
        });
      ...
      this.process.on('error', (err) => {
        ...
        logger.debug('[AcpBackend] Provider process error');
      ...
      this.process.on('exit', (code) => {
        if (!this.disposed && code !== 0 && code !== null) {
          ...
          logger.debug('[AcpBackend] Provider process exited unsuccessfully');
          ...
          try {
            while (true) {
              ...
              if (done) {
                ...
                if (filteredCount > 0) {
                  logger.debug('[AcpBackend] Filtered non-JSON provider output', {
                    lineCount: boundedDiagnosticCount(filteredCount),
                  });
                }
                controller.close();
                break;
            ...
            logger.debug('[AcpBackend] Provider stdout filtering failed');
      ...
      const client: Client = {
        ...
        requestPermission: async (params: RequestPermissionRequest): Promise<RequestPermissionResponse> => {
          ...
          if (toolName !== (toolCall?.kind || toolCall?.toolName || extendedParams.kind || 'Unknown tool')) {
            logger.debug('[AcpBackend] Normalized a provider tool name');
          }
          ...
          logger.debug('[AcpBackend] Permission request received', {
            hasToolCall: !!toolCall,
            inputFieldCount: boundedDiagnosticCount(Object.keys(input).length),
            optionCount: boundedDiagnosticCount(options.length),
            parameterFieldCount: boundedDiagnosticCount(Object.keys(params).length),
          });
          ...
          if (this.options.permissionHandler) {
            try {
              ...
              logger.debug('[AcpBackend] Permission handler failed');
      ...
      logger.debug('[AcpBackend] Initializing connection', {
        timeoutMs: boundedDiagnosticCount(initTimeout),
      });
      ...
      if (this.options.verbose) {
        logAcpBackendMuted(
          `Incoming initialize response metadata: ${summarizeSessionMetadataPayload(initializeResponse)}`,
        );
      }
      ...
      logger.debug('[AcpBackend] Session created');
      if (this.options.verbose) {
        logAcpBackendMuted(
          `Incoming newSession response metadata: ${summarizeSessionMetadataPayload(sessionResponse)}`,
        );
      }
      ...
      if (initialPrompt) {
        this.sendPrompt(sessionId, initialPrompt).catch((error) => {
          // Log to file only, not console
          logger.debug('[AcpBackend] Initial prompt send failed');
      ...
      try {
        ...
        logger.warn('[AcpBackend] Failed to reset sandbox after startup error');
      }
      // Log to file only, not console
      logger.debug('[AcpBackend] Session startup failed');
  ...
  private handleSessionUpdate(params: SessionNotification): void {
    ...
    if (!update) {
      logger.debug('[AcpBackend] Session notification omitted its update field', {
        parameterFieldCount: boundedDiagnosticCount(Object.keys(params).length),
      });
      return;
    }
    ...
    const updateFieldCount = boundedDiagnosticCount(Object.keys(update).length);
    logger.debug('[AcpBackend] Session update received', { updateFieldCount });
    if (this.options.verbose) {
      logAcpBackendMuted(
        `Incoming session update metadata: fieldCount=${updateFieldCount}`,
      );
    }
    ...
    if (updateType &&
        ...
        !update.plan &&
        !update.thinking) {
      logger.debug('[AcpBackend] Unhandled session update received', { updateFieldCount });
    }
  }
  ...
  async sendPrompt(sessionId: SessionId, prompt: string): Promise<void> {
    ...
    try {
      logger.debug('[AcpBackend] Sending prompt', {
        characterCount: boundedDiagnosticCount(prompt.length),
      });
      ...
      logger.debug('[AcpBackend] Prompt request failed');
  ...
  async setSessionConfigOption(configId: string, value: string): Promise<boolean> {
    ...
    try {
      ...
      logger.debug('[AcpBackend] Failed to set a session config option');
      return false;
    }
  }
  ...
  async setSessionMode(modeId: string): Promise<boolean> {
    ...
    try {
      ...
      logger.debug('[AcpBackend] Failed to set the session mode');
      return false;
    }
  }
  ...
  async setSessionModel(modelId: string): Promise<boolean> {
    ...
    try {
      ...
      logger.debug('[AcpBackend] Failed to set the session model');
      return false;
    }
  }
  ...
  async cancel(sessionId: SessionId): Promise<void> {
    ...
    try {
      ...
      logger.debug('[AcpBackend] Session cancellation failed');
    }
  }
  ...
  async respondToPermission(requestId: string, approved: boolean): Promise<void> {
    logger.debug('[AcpBackend] Permission response event emitted', { approved });
  ...
  async dispose(): Promise<void> {
    ...
    if (this.connection && this.acpSessionId) {
      try {
        ...
        logger.debug('[AcpBackend] Graceful provider shutdown failed');
      }
    }
    ...
    try {
      ...
      logger.warn('[AcpBackend] Failed to reset sandbox after provider exit');
    }
  }
}
