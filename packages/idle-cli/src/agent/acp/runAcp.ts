...
class GenericAcpPermissionHandler extends BasePermissionHandler implements AcpPermissionHandler {
  ...
  constructor(session: ApiSessionClient, _agentName: string) {
    super(session);
    this.logPrefix = '[ACP]';
  }
  ...
  async handleToolCall(toolCallId: string, toolName: string, input: unknown): Promise<PermissionResult> {
    return new Promise<PermissionResult>((resolve, reject) => {
      ...
      logger.debug(`${this.logPrefix} Permission request queued`, {
...
export async function runAcp(opts: {
  ...
  // Start with no sandbox claim. The backend updates this only after the OS
  // wrapper has initialized and the provider spawn has been prepared.
  ...
  if (response) {
    try {
      ...
      logger.debug('[ACP] Failed to report session to daemon', errorLogMetadata(error));
    }
  }
  ...
  const stopRunnerFromBackendStatus = (status: 'error' | 'stopped', detail?: string) => {
    ...
    logger.debug('[ACP] Terminal provider status received; stopping ACP runner', {
      status,
      hasDetail: typeof detail === 'string' && detail.length > 0,
    });
    shouldExit = true;
    messageQueue.close();
  ...
  const switchPermissionModeIfRequested = async (requestedMode: string): Promise<void> => {
    ...
    if (modeSelector) {
      ...
      if (!resolved) {
        logger.debug('[ACP] Ignoring unknown permission mode request');
        return;
      }
    ...
    if (!resolvedLegacyMode) {
      logger.debug('[ACP] Ignoring unknown legacy permission mode request');
      return;
    }
  ...
  const switchModelIfRequested = async (requestedModel: string): Promise<void> => {
    ...
    if (modelSelector) {
      ...
      if (!resolved) {
        logger.debug('[ACP] Ignoring unknown model request');
        return;
      }
    ...
    if (!resolvedLegacyModel) {
      logger.debug('[ACP] Ignoring unknown legacy model request');
      return;
    }
  ...
  session.onUserMessage((message) => {
    ...
    if (typeof message.meta?.permissionMode === 'string') {
      ...
      logger.debug('[ACP] Permission mode requested', { hasValue: currentPermissionMode.length > 0 });
    }

    if (message.meta && Object.prototype.hasOwnProperty.call(message.meta, 'model')) {
      currentModel = message.meta.model ?? null;
      logger.debug('[ACP] Model requested', { hasValue: typeof currentModel === 'string' && currentModel.length > 0 });
    }
  ...
  async function handleAbort() {
    try {
      ...
      logger.debug('[ACP] Abort failed', errorLogMetadata(error));
    } finally {
  ...
  try {
    ...
    try {
      ...
      logger.debug('[ACP] Failed to reset permission handler', errorLogMetadata(error));
    }
    ...
    try {
      ...
      logger.debug('[ACP] Failed to stop Idle MCP server', errorLogMetadata(error));
    }
    ...
    try {
      ...
      logger.debug('[ACP] Session close failed', errorLogMetadata(error));
    }
  }
  } finally {
    try {
      ...
      logger.debug('[ACP] Failed to release Idle MCP ownership');
    }
  }
}
