/** ACP session-update validation, state tracking, and event translation. */
...
export function handleAgentMessageChunk(
  ...
  if (isThinking) {
    ...
    logger.debug('[AcpBackend] Received message chunk', {
      characterCount: boundedDiagnosticCount(text.length),
    });
    ctx.emit({
      type: 'model-output',
      textDelta: text,
    ...
    ctx.setIdleTimeout(() => {
      if (ctx.activeToolCalls.size === 0) {
        ...
        logger.debug('[AcpBackend] Delaying idle status', {
          activeToolCallCount: boundedDiagnosticCount(ctx.activeToolCalls.size),
        });
      }
    }, idleTimeoutMs);
  }
...
export function handleAgentThoughtChunk(
  ...
  if (ctx.activeToolCalls.size > 0) {
    logger.debug('[AcpBackend] Thinking chunk received during active tool calls', {
      chunkLength: boundedDiagnosticCount(text.length),
      activeToolCallCount: boundedDiagnosticCount(ctx.activeToolCalls.size),
    });
  }
...
export function startToolCall(
  ...
  logger.debug('[AcpBackend] Tool call started', {
    source,
    isInvestigation,
    activeToolCallCount: boundedDiagnosticCount(ctx.activeToolCalls.size),
  });
  ...
  if (isInvestigation) {
    logger.debug('[AcpBackend] Extended tool timeout selected');
  }
  ...
  if (!ctx.toolCallTimeouts.has(toolCallId)) {
    const timeout = setTimeout(() => {
      logger.debug('[AcpBackend] Tool call timed out', {
        source,
        timeoutSeconds: boundedDiagnosticCount(Math.round(timeoutMs / 1000)),
      });
    ...
    logger.debug('[AcpBackend] Set tool call timeout', {
      timeoutSeconds: boundedDiagnosticCount(Math.round(timeoutMs / 1000)),
      isInvestigation,
    });
  } else {
    logger.debug('[AcpBackend] Tool call timeout already set; skipping');
  }
...
export function completeToolCall(
  ...
  logger.debug('[AcpBackend] Tool call completed', {
    activeToolCallCount: boundedDiagnosticCount(ctx.activeToolCalls.size),
  });
...
export function failToolCall(
  ...
  if (timeout) {
    clearTimeout(timeout);
    ctx.toolCallTimeouts.delete(toolCallId);
    logger.debug('[AcpBackend] Cleared tool call timeout', { status });
  } else {
    logger.debug('[AcpBackend] No timeout found for ended tool call', { status });
  }

  logger.debug('[AcpBackend] Tool call failed or was cancelled', {
    status,
    isInvestigation,
    hadTimeout,
    contentBlockCount: boundedDiagnosticCount(Array.isArray(content) ? content.length : 0),
    activeToolCallCount: boundedDiagnosticCount(ctx.activeToolCalls.size),
  });
  ...
  if (errorDetail) {
    logger.debug('[AcpBackend] Tool failure detail received', {
      characterCount: boundedDiagnosticCount(errorDetail.length),
    });
  } else {
    logger.debug('[AcpBackend] Tool failure detail absent', { status });
  }
...
export function handleToolCallUpdate(
  ...
  if (!toolCallId) {
    logger.debug('[AcpBackend] Tool call update without tool call ID', {
      hasStatus: typeof update.status === 'string' && update.status.length > 0,
      hasKind: typeof update.kind === 'string' && update.kind.length > 0,
      contentType: valueType(update.content),
    });
    return { handled: false };
  }
  ...
  if (status === 'in_progress' || status === 'pending') {
    if (!ctx.activeToolCalls.has(toolCallId)) {
      ...
      logger.debug('[AcpBackend] Tool call already tracked', {
        activeToolCallCount: boundedDiagnosticCount(ctx.activeToolCalls.size),
      });
    }
...
export function handleToolCall(
  ...
  logger.debug('[AcpBackend] Received tool call', {
    hasToolCallId: typeof toolCallId === 'string' && toolCallId.length > 0,
    hasStatus: typeof status === 'string' && status.length > 0,
    hasKind: typeof update.kind === 'string' && update.kind.length > 0,
  });
  ...
  if (!toolCallId || !isInProgress) {
    logger.debug('[AcpBackend] Tool call is not in progress; skipping', {
      hasToolCallId: typeof toolCallId === 'string' && toolCallId.length > 0,
      hasStatus: typeof status === 'string' && status.length > 0,
    });
    return { handled: false };
  ...
  if (ctx.activeToolCalls.has(toolCallId)) {
    logger.debug('[AcpBackend] Tool call already active; skipping', {
      activeToolCallCount: boundedDiagnosticCount(ctx.activeToolCalls.size),
    });
    return { handled: true };
  }
