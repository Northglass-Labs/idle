...
import { checkIfDaemonRunningAndCleanupStaleState, isDaemonRunningCurrentlyInstalledIdleVersion } from './controlClient'
...
export async function ensureDaemonRunning(): Promise<void> {
  ...
  // silently lost — which later breaks resume-idle-session.
  const deadline = Date.now() + DAEMON_READY_TIMEOUT_MS
  while (Date.now() < deadline) {
    if (await checkIfDaemonRunningAndCleanupStaleState()) {
      logger.debug('Idle background service is ready')
      return
    }
    await new Promise(resolve => setTimeout(resolve, DAEMON_READY_POLL_INTERVAL_MS))
  }

  logger.debug(`Idle background service did not become ready within ${DAEMON_READY_TIMEOUT_MS}ms; continuing anyway`)
}
