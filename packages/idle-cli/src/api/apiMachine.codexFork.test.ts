...
vi.mock('@/codex/codexAppServerClient', () => ({
    // vitest 4: `new` requires a constructible impl — arrow functions throw.
    CodexAppServerClient: vi.fn(function (this: unknown) { return codexClientMethods; }),
}));
...
describe('ApiMachineClient Codex fork RPCs', () => {
    ...
    it('forwards resumeCodexThreadId through the spawn RPC', async () => {
        const spawnSession = vi.fn().mockResolvedValue({ type: 'success', sessionId: 'idle-forked' });
        ...
        const result = await handlersFrom(client).get('machine-1:spawn-idle-session')?.({
            directory: '/tmp/project',
            agent: 'codex',
