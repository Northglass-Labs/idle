...
import {
    RemoteSpawnSessionRequestSchema,
    SpawnSessionOptions,
    SpawnSessionResult,
} from '../daemon/spawnSessionOptions';
import { encodeBase64, decodeBase64, encrypt, decrypt } from './encryption';
import { backoff } from '@/utils/time';
import { RpcHandlerManager } from './rpc/RpcHandlerManager';
import { detectCLIAvailability, CLIAvailability } from '@/utils/detectCLI';
import { detectResumeSupport, type ResumeSupport } from '@/resume/localIdleAgentAuth';
...
import { isValidClaudeSessionId } from '@/claude/utils/claudeSessionId';
...
export class ApiMachineClient {
    ...
    constructor(
        ...
        private rpcRegistrationToken: string = token,
    ) {
        ...
        this.rpcHandlerManager = new RpcHandlerManager({
            ...
            logger: (msg, data) => logger.debug(msg, {
                hasMetadata: data !== undefined,
            })
        });

        registerCommonHandlers(this.rpcHandlerManager, process.cwd());
        this.rpcHandlerManager.registerHandler('machine-home-directory', async (params: unknown) => {
            if (
                params === null
                ...
                throw new Error('Invalid machine home request');
            }
            const directory = homedir();
            if (
                directory.length === 0
                || directory.length > 4 * 1024
                || /[\u0000-\u001f\u007f]/u.test(directory)
                || !isAbsolute(directory)
            ) {
                throw new Error('Machine home directory is unavailable');
            }
            return { directory };
        });
    }
    ...
    setRPCHandlers({
        ...
        this.rpcHandlerManager.registerHandler('spawn-idle-session', async (params: unknown) => {
            const parsed = RemoteSpawnSessionRequestSchema.safeParse(params);
            if (!parsed.success) {
                throw new Error('Invalid spawn session request');
            }
            const { type: _requestType, ...spawnOptions } = parsed.data;
            const { directory, agent, resumeClaudeSessionId } = spawnOptions;
            if (resumeClaudeSessionId !== undefined && !isValidClaudeSessionId(resumeClaudeSessionId)) {
                throw new Error('resumeClaudeSessionId must be a valid UUID');
            }
            logger.debug('[API MACHINE] Spawning session', {
                hasExplicitAgent: agent !== undefined,
            ...
            switch (result.type) {
                case 'success':
                    logger.debug('[API MACHINE] Session spawned');
                ...
                case 'requestToApproveCodexNativeSandbox':
                    logger.debug('[API MACHINE] Codex native sandbox approval required');
        ...
        this.rpcHandlerManager.registerHandler('stop-session', async (params: any) => {
            ...
            logger.debug('[API MACHINE] Session stopped');
        ...
        // Claude UUID. The caller then spawns a fresh Idle session with
        ...
        this.rpcHandlerManager.registerHandler('claude-duplicate-session', async (params: any) => {
            ...
            if (!isValidClaudeSessionId(cutAfterUuid)) {
    ...
    private syncResumeSessionRpcRegistration(): void {
        const method = 'resume-idle-session';
    ...
    /** Update encrypted machine metadata with optimistic concurrency. */
    ...
    connect() {
        ...
        logger.debug('[API MACHINE] Connecting to relay');
        ...
        this.socket.on('disconnect', (reason) => {
            logger.debug('[API MACHINE] Disconnected from relay', {
        ...
        this.socket.on('rpc-request', async (data: { method: string, params: string }, callback: (response: string) => void) => {
            logger.debug('[API MACHINE] Received RPC request', {
                ciphertextLength: typeof data.params === 'string'
                    ? Math.min(data.params.length, 16 * 1024 * 1024)
                    : 0,
            });
        ...
        this.socket.on('update', (data: Update) => {
            ...
            if (data.body.t === 'update-machine' && (data.body as UpdateMachineBody).machineId === this.machine.id) {
                ...
                logger.debug('[API MACHINE] Ignored an unknown update type');
            }
        });
    ...
    private startKeepAlive() {
        ...
        this.keepAliveInterval = setInterval(() => {
            ...
            if (process.env.DEBUG) {
                logger.debug('[API MACHINE] Emitting keep-alive');
            }
            ...
            const resumeSupportChanged = !prevResume
                ...
                || prevResume.idleAgentAuthenticated !== newResumeSupport.idleAgentAuthenticated;
            ...
            if (cliAvailabilityChanged || resumeSupportChanged) {
                ...
                this.updateMachineMetadata((metadata) => ({
                    ...
                    logger.debug('[API MACHINE] Machine capability update failed');
                });
            }
        }, 20000);
