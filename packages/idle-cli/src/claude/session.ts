...
export class Session {
    ...
    /** Synchronize a discovered provider session reference and notify observers. */
    onSessionFound = (sessionId: string) => {
        ...
        if (previous === null) {
            logger.debug('[Session] Provider session reference initialized');
        } else if (previous !== sessionId) {
            logger.debug('[Session] Provider session reference changed');
        }
    ...
    consumeOneTimeFlags = (): void => {
            ...
            if (arg === '--resume') {
                ...
                if (i + 1 < this.claudeArgs.length) {
                    ...
                    if (!nextArg.startsWith('-') && nextArg.includes('-')) {
                        ...
                        i++; // Skip the UUID
                        logger.debug('[Session] Consumed --resume flag with a session reference');
                    } else {
        ...
        logger.debug('[Session] Consumed one-time flags', {
            remainingArgumentCount: this.claudeArgs?.length ?? 0,
        });
    }
}
