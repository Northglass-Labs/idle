...
const {
    ...
    mockCleanupHookSettingsFile,
    mockGenerateHookSettingsFile,
    mockLoop,
    ...
    mockGenerateHookSettingsFile: vi.fn(() => '/tmp/idle-hook-settings.json'),
    mockLoop: vi.fn(),
...
vi.mock('@/claude/utils/startIdleServer', () => ({
    startIdleServer: mockStartIdleServer,
}));
...
vi.mock('@/claude/utils/generateHookSettings', () => ({
    generateHookSettingsFile: mockGenerateHookSettingsFile,
    cleanupHookSettingsFile: mockCleanupHookSettingsFile,
}));
...
vi.mock('@/ui/logger', () => ({
    logger: {
        ...
        infoDeveloper: vi.fn(),
        // Idle's runClaude also logs at info/log level (SESS-ID-1 hook-source logging).
        info: vi.fn(),
        log: vi.fn(),
    },
}));
...
describe('runClaude remote JSONL scanner', () => {
    ...
    const forkSessionEnvKey = 'IDLE_FORK_CLAUDE_SESSION_ID';
    const originalListeners = new Map<string, Array<(...args: any[]) => void>>();

    beforeEach(() => {
        vi.clearAllMocks();
        mockGenerateHookSettingsFile.mockReturnValue('/tmp/idle-hook-settings.json');
        for (const event of processEvents) {
            originalListeners.set(event, process.listeners(event as any) as Array<(...args: any[]) => void>);
        }

        delete process.env.IDLE_RECONNECT_SESSION_ID;
        delete process.env.IDLE_RECONNECT_ENCRYPTION_KEY;
        delete process.env.IDLE_RECONNECT_ENCRYPTION_VARIANT;
        delete process.env.IDLE_RECONNECT_SEQ;
        delete process.env.IDLE_RECONNECT_METADATA_VERSION;
        delete process.env.IDLE_RECONNECT_AGENT_STATE_VERSION;
        delete process.env.IDLE_FORKED_FROM_SESSION_ID;
        delete process.env.IDLE_FORKED_FROM_MESSAGE_ID;
        delete process.env.IDLE_FORK_CLAUDE_SESSION_ID;
        ...
        mockStartIdleServer.mockResolvedValue({
            url: 'http://127.0.0.1:12345',
            tokenFilePath: '/private/idle/session.token',
            toolNames: ['change_title'],
            stop: vi.fn(),
            // Idle feature: auto-title fallback (idempotent first-message scheduling).
    ...
    it('keeps the authenticated sender ID attached to the exact queued remote prompt', async () => {
        const harness = await startRemoteRunClaudeHarness();
        const userMessageHandler = harness.sessionClient.onUserMessage.mock.calls[0][0];

        await userMessageHandler({
            role: 'user',
            content: { type: 'text', text: 'Correlate this turn' },
            meta: {},
            messageIdentity: {
                v: 1,
                sessionId: 'idle-session-1',
                messageId: 'sender-owned-request-claude',
            },
        });
        ...
        expect(harness.loopOptions.messageQueue.queue).toEqual([
            expect.objectContaining({
                ...
                mode: expect.objectContaining({
                    requestId: 'sender-owned-request-claude',
                }),
            }),
        ]);
    ...
    it('releases the MCP capability and scanner when hook startup fails', async () => {
        const stopIdleServer = vi.fn();
        const cleanupScanner = vi.fn(async () => {});
        const sessionClient = {
            getMetadata: vi.fn(() => ({})),
            sendClaudeSessionMessage: vi.fn(),
        ...
        mockStartHookServer.mockRejectedValue(new Error('hook startup failed'));
        ...
        await expect(runClaude({
            ...
            shouldStartDaemon: false,
            startingMode: 'remote',
        })).rejects.toThrow('hook startup failed');

        expect(stopIdleServer).toHaveBeenCalledOnce();
        expect(cleanupScanner).toHaveBeenCalledOnce();
    });

    it.each([
        '../../outside-project/session',
        'not-a-uuid',
    ])('does not read a Claude fork transcript for invalid session id %j', async (forkClaudeSessionId) => {
        process.env[forkSessionEnvKey] = forkClaudeSessionId;
        mockReadFile.mockResolvedValue(JSON.stringify({
            type: 'user',
            uuid: 'unexpected-user-message',
        ...
        expect(harness.sessionClient.sendClaudeSessionMessageFromLocalTranscript).not.toHaveBeenCalled();
        await harness.finish();
    });

    it('backfills a valid Claude fork UUID and binds it to session metadata', async () => {
        const forkClaudeSessionId = '123e4567-e89b-42d3-a456-426614174000';
        process.env[forkSessionEnvKey] = forkClaudeSessionId;
        const transcriptMessage = {
            type: 'user',
            uuid: 'historical-user-message',
            message: { content: 'preserved fork context' },
        };
        mockReadFile.mockResolvedValue(`${JSON.stringify(transcriptMessage)}\n`);

        const harness = await startRemoteRunClaudeHarness();

        expect(mockReadFile).toHaveBeenCalledOnce();
        expect(mockReadFile.mock.calls[0]?.[0]).toEqual(expect.stringMatching(
            new RegExp(`${forkClaudeSessionId}\\.jsonl$`),
        ));
        expect(harness.sessionClient.sendClaudeSessionMessageFromLocalTranscript)
            .toHaveBeenCalledWith(transcriptMessage);
        const metadataUpdater = harness.sessionClient.updateMetadata.mock.calls
            .map(([updater]) => updater)
            .find((updater) => updater({}).claudeSessionId === forkClaudeSessionId);
        expect(metadataUpdater).toEqual(expect.any(Function));
        await harness.finish();
    });
