...
import { applySandboxPermissionPolicy, hasExplicitClaudeBypassArg, resolveInitialClaudePermissionMode, resolveRemoteClaudePermissionMode } from './utils/permissionMode';
...
import { isValidClaudeSessionId } from './utils/claudeSessionId';
...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    logger.debug('[START] Idle process started');
    ...
    const preserveExplicitBypass = hasExplicitClaudeBypassArg(options.claudeArgs);
    ...
    logger.debug('[Claude] Using the configured machine identity');
    ...
    let metadata: Metadata = {
        ...
        flavor: 'claude',
        // metadata.sandbox is the SECURITY CLAIM the mobile UI badges on.
        // Start null — claudeLocal will push an updateMetadata once sandbox
        // wrapping is verified (via the onSandboxApplied callback below).
        // Remote/SDK mode never applies the sandbox runtime, so this stays null
        // for the entire mobile-controlled session. See docs/permission-resolution.md.
        sandbox: null,
    ...
    if (reconnectSessionId && reconnectKeyBase64 && reconnectVariant) {
        logger.debug('[START] Reconnecting to an existing Claude session');
        response = {
    ...
    if (!response) {
        ...
        const reconnection = startOfflineReconnection({
            ...
            onReconnected: async () => {
                // Reuse the original sessionTag rather than rolling a fresh randomUUID.
                // Server uses tag-based dedup (sessionRoutes.ts:233-238 findFirst by tag); a
                // fresh tag here means the reconnect creates a SECOND server session record for
                // the same CLI process. Over multiple network blips that compounds into N "ghost"
                // sessions all showing as active, each receiving heartbeats. Reusing the
                // original tag prevents duplicate records across network interruptions.
        ...
        try {
            await claudeLocal({
                ...
                allowedTools: [],
                sandboxConfig,
                // No active session in the offline-reconnect path; honest sandbox
                // status is conveyed once reconnection.onReconnected creates one.
                onSandboxApplied: undefined,
            });
        } finally {
            reconnection.cancel();
        }
    ...
    logger.info('[SESSION] Idle session created');

    // Always report to daemon if it exists
    try {
        logger.debug('[START] Reporting Claude session to daemon');
        ...
        if (result.error) {
            logger.debug('[START] Failed to report Claude session to daemon (may not be running)');
        } else {
            logger.debug('[START] Reported Claude session to daemon');
        }
    } catch {
        logger.debug('[START] Failed to report Claude session to daemon (may not be running)');
    }

    // SDK metadata comes from the system.init message via onSDKMetadata.
    ...
    // Fork backfill: when this Idle session was just spawned as a fork
    // of another (IDLE_FORK_CLAUDE_SESSION_ID is set by the daemon at
    ...
    // historical messages back to the Idle client — so without an
    ...
    //
    // Skipped on reconnect (IDLE_RECONNECT_*) — that path reattaches
    // to the existing Idle session, where the server already has every
    // message it needs.
    const forkClaudeSessionId = process.env.IDLE_FORK_CLAUDE_SESSION_ID;
    if (!reconnectSessionId && forkClaudeSessionId && !isValidClaudeSessionId(forkClaudeSessionId)) {
        logger.debug('[FORK BACKFILL] Ignoring invalid Claude session ID');
    }
    if (!reconnectSessionId && isValidClaudeSessionId(forkClaudeSessionId)) {
        ...
        try {
            ...
            logger.debug('[FORK BACKFILL] Replayed historical messages', {
                messageCount: backfilled,
            });
            // Bind the new Idle session to the forked Claude UUID up
            ...
            logger.debug('[FORK BACKFILL] Failed to read the source transcript');
        }
    }
    ...
    // alongside this Idle session. App-sent prompts also land in the
    ...
    // to the running Idle session. SDK-emitted assistant + tool_result
    ...
    logger.debug('[START] Idle MCP server started');
    ...
    let localResourcesReleased = false;

    const releaseLocalResources = async (): Promise<void> => {
        if (localResourcesReleased) return;
        localResourcesReleased = true;

        try {
            idleServer.stop();
        } catch {
            logger.debug('[START] Idle MCP cleanup failed');
        }
        try {
            hookServer?.stop();
        } catch {
            logger.debug('[START] Hook server cleanup failed');
        }
        if (hookSettingsPath) {
        ...
        try {
            ...
            logger.debug('[START] Session scanner cleanup failed');
        }
    };
    ...
    hookServer = await startHookServer({
        onSessionHook: (sessionId, data) => {
            logger.info('[SESSION] Claude SessionStart hook received', {
            ...
            // is therefore already on the server and must not be replayed.
            // The scanner's job is forwarding
            ...
            if (currentSession) {
                ...
                if (previousSessionId !== sessionId) {
                    ...
                    // The Session.onSessionFound already emits a [SESS-ID-1]
                    // mapping log line; no need to duplicate here.
                }
            }
        }
    });
    logger.debug('[START] Hook server started');

    // Generate hook settings file for Claude
    hookSettingsPath = generateHookSettingsFile(hookServer.port, hookServer.authToken);
    logger.debug('[START] Generated owner-only hook settings');

    logger.infoDeveloper('Claude session initialization complete');
    ...
    const currentEnhancedMode = (requestId?: string): EnhancedMode => ({
    ...
    session.onFileEvent((fileEvent) => {
        ...
        logger.debug('[loop] File event received', {
            declaredBytes: ev.size,
        ...
        const downloadPromise = (async (): Promise<{ data: Uint8Array; mimeType: string; name: string } | null> => {
            try {
                ...
                logger.debug('[loop] Attachment decrypted', { bytes: decrypted.length });
    ...
    session.onUserMessage(async (message) => {
        ...
        if (message.meta?.permissionMode) {
            ...
            if (ignoredDefaultDowngrade) {
                logger.debug('[loop] Ignoring a permission-mode downgrade to default');
            } else {
                logger.debug('[loop] Permission mode updated from user message');
            }
        } else {
            logger.debug('[loop] User message received without a permission-mode override');
        }
        ...
        if (message.meta?.hasOwnProperty('model')) {
            ...
            logger.debug('[loop] Model selection updated from user message', {
                hasModel: typeof messageModel === 'string',
            });
        } else {
            logger.debug('[loop] User message received without a model override');
        }
        ...
        if (message.meta?.hasOwnProperty('fallbackModel')) {
            ...
            logger.debug('[loop] Fallback model selection updated from user message', {
                hasFallbackModel: typeof messageFallbackModel === 'string',
            });
        } else {
            logger.debug('[loop] User message received without a fallback-model override');
        }
        ...
        if (message.meta?.hasOwnProperty('allowedTools')) {
            ...
            logger.debug('[loop] Allowed-tool policy updated from user message', {
                toolCount: messageAllowedTools?.length ?? 0,
            });
        } else {
            logger.debug('[loop] User message received without an allowed-tools override');
        }
        ...
        if (message.meta?.hasOwnProperty('disallowedTools')) {
            ...
            logger.debug('[loop] Disallowed-tool policy updated from user message', {
                toolCount: messageDisallowedTools?.length ?? 0,
            });
        } else {
            logger.debug('[loop] User message received without a disallowed-tools override');
        }
        ...
        if (message.meta?.hasOwnProperty('effort')) {
            ...
            if (incoming === null || incoming === undefined) {
                ...
                logger.debug('[loop] Reasoning effort updated from user message');
            } else {
                logger.debug('[loop] Ignoring an invalid reasoning effort from a user message');
            }
        } else {
            logger.debug('[loop] User message received without a reasoning-effort override');
        }
        ...
        if (specialCommand.type === 'compact') {
            ...
            messageQueue.pushIsolateAndClear(specialCommand.originalMessage || message.content.text, currentEnhancedMode(message.messageIdentity?.messageId), attachmentsForThisMessage);
            logger.debug('[start] /compact command pushed to an isolated queue');
            return;
        }
        ...
        if (specialCommand.type === 'clear') {
            ...
            messageQueue.pushIsolateAndClear(specialCommand.originalMessage || message.content.text, currentEnhancedMode(message.messageIdentity?.messageId), attachmentsForThisMessage);
            logger.debug('[start] /clear command pushed to an isolated queue');
            return;
        }
        ...
        messageQueue.push(message.content.text, currentEnhancedMode(message.messageIdentity?.messageId), attachmentsForThisMessage);
        logger.debug('User message pushed to queue', {
            textBytes: Buffer.byteLength(message.content.text, 'utf8'),
            attachmentCount: attachmentsForThisMessage.length,
        })
    });
        ...
        try {
            ...
            if (session) {
                ...
                try {
                    ...
                    logger.debug('[START] deactivateSession during cleanup failed');
                }
            ...
            logger.debug('[START] Error during cleanup');
            process.exit(1);
        }
    };
    ...
    process.on('uncaughtException', () => {
        logger.debug('[START] Uncaught exception');
        void cleanup({ archive: true });
    });

    process.on('unhandledRejection', () => {
        logger.debug('[START] Unhandled rejection');
        void cleanup({ archive: true });
    });
    ...
    const exitCode = await loop({
        ...
        onModeChange: (newMode) => {
            ...
            // Mode flipping into 'remote' means the local sandbox is no longer
            // in effect for whatever spawns next. Clear the security claim so
            // the UI doesn't continue to badge as sandboxed once we leave local.
            if (newMode === 'remote') {
                session.updateMetadata((m) => ({ ...m, sandbox: null }));
            }
        },
    ...
    await releaseLocalResources();
    logger.debug('Stopped local session resources');
