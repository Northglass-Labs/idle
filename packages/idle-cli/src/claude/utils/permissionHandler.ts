...
const UNSAFE_REMEMBERED_SHELL_SYNTAX = /[\0\r\n`]|\$\(/;

function parseSimpleShellWords(command: string): string[] | null {
    if (UNSAFE_REMEMBERED_SHELL_SYNTAX.test(command)) {
        return null;
    }

    try {
        const entries = parseShellCommand(command, (name) => `$${name}`);
...
function matchesRememberedBashPrefix(command: string, prefix: string): boolean {
    const commandWords = parseSimpleShellWords(command);
    const prefixWords = parseSimpleShellWords(prefix);
    if (!commandWords || !prefixWords || commandWords.length < prefixWords.length) {
        return false;
    }
    return prefixWords.every((word, index) => commandWords[index] === word);
}
...
export class PermissionHandler {
    ...
    private handlePermissionResponse(
        ...
        if (pending.toolName === 'exit_plan_mode' || pending.toolName === 'ExitPlanMode') {
            logger.debug('Plan mode result received', {
                ...
                hasReason: Boolean(response.reason),
                allowToolCount: response.allowTools?.length ?? 0,
                hasUpdatedInput: Boolean(response.updatedInput),
            });
            if (response.approved) {
                ...
                logger.debug('Plan approved; updating permission mode and allowing ExitPlanMode');

                if (this.setPermissionModeCallback) {
                    this.setPermissionModeCallback(newMode).catch(() => {
                        logger.debug('Failed to set permission mode via SDK');
                    });
                }
    ...
    handleToolCall = async (toolName: string, input: unknown, mode: EnhancedMode, options: { signal: AbortSignal; toolUseID: string }): Promise<PermissionResult> => {
        ...
        if (toolName === 'Bash') {
            ...
            if (inputObj?.command) {
                ...
                for (const prefix of this.allowedBashPrefixes) {
                    if (matchesRememberedBashPrefix(inputObj.command, prefix)) {
                        return { behavior: 'allow', updatedInput: input as Record<string, unknown> };
                    }
                }
            }
        ...
        // Leaving plan mode always requires explicit approval; a permission mode
        // inherited from another turn cannot authorize plan execution.
    ...
    private async handlePermissionRequest(
        ...
        return new Promise<PermissionResult>((resolve, reject) => {
            ...
            logger.debug('Permission request sent', {
                inputFieldCount: input && typeof input === 'object' && !Array.isArray(input)
    ...
    private setupClientHandler(): void {
        this.session.client.rpcHandlerManager.registerHandler<PermissionResponse, void>('permission', async (message) => {
            logger.debug('Permission response received', {
                ...
                hasReason: Boolean(message.reason),
            });
