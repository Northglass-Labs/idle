/**
 * Fork the on-disk Claude session JSONL so a brand-new Idle session can
...
export async function forkSession(projectDir: string, sourceClaudeSessionId: string): Promise<string> {
    ...
    logger.debug('[CLAUDE FORK] Session fork created');
    return newId;
}
...
export async function forkAndTruncateSession(
    ...
    logger.debug('[CLAUDE FORK] Truncated session fork created');
    return newId;
}
