...
export async function createSessionScanner(opts: {
    ...
    // and spins the CPU while flooding the log.
    ...
    if (opts.sessionId) {
        ...
        logger.debug('[SESSION_SCANNER] Marking existing entries as processed', {
    ...
    const sync = new InvalidateSync(async () => {
        ...
        for (let session of sessions) {
            ...
            for (let entry of sessionEntries) {
                ...
                if (entry.kind === 'message') {
                    logger.debug('[SESSION_SCANNER] Sending a new transcript message');
                    opts.onMessage(entry.message);
                    sentMessages++;
                } else {
                    logger.debug('[SESSION_SCANNER] Sending a new transcript event');
                    opts.onTranscriptEvent?.(entry.event);
                    sentTranscriptEvents++;
                }
            }
            if (sessionEntries.length > 0) {
                logger.debug('[SESSION_SCANNER] Session scan completed', {
                    foundCount: sessionEntries.length,
                    skippedCount: skipped,
                    sentMessageCount: sentMessages,
                    sentTranscriptEventCount: sentTranscriptEvents,
                });
            }
        }
        ...
        for (let p of sessions) {
            if (!watchers.has(p) && !deadSessions.has(p)) {
                logger.debug('[SESSION_SCANNER] Starting a session watcher');
                watchers.set(p, startFileWatcher(
                    ...
                    () => { sync.invalidate(); },
                    {
                        ...
                        onGaveUp: () => {
                            ...
                            logger.debug('[SESSION_SCANNER] Session transcript never appeared; dropping its watcher');
                            watchers.get(p)?.();
                            watchers.delete(p);
                            deadSessions.add(p);
                            pendingSessions.delete(p);
    ...
    return {
        ...
        onNewSession: async (sessionId: string, options?: { treatExistingAsProcessed?: boolean }) => {
            if (currentSessionId === sessionId) {
                logger.debug('[SESSION_SCANNER] Duplicate current session announcement ignored');
                return;
            }
            // An explicit re-announcement makes a deferred session eligible
            // again because its file may now exist.
            if (deadSessions.delete(sessionId)) {
                logger.debug('[SESSION_SCANNER] Reviving a deferred session watcher');
            }
            if (finishedSessions.has(sessionId)) {
                logger.debug('[SESSION_SCANNER] Finished session announcement ignored');
                return;
            }
            if (pendingSessions.has(sessionId)) {
                logger.debug('[SESSION_SCANNER] Pending session announcement ignored');
                return;
            }
            // When the caller already has these messages (typical for
            // On reconnect the server holds the history from prior
            ...
            if (options?.treatExistingAsProcessed) {
                ...
                logger.debug('[SESSION_SCANNER] Pre-marking existing entries as processed', {
            ...
            logger.debug('[SESSION_SCANNER] New session announced')
...
async function readSessionEntries(projectDir: string, sessionId: string): Promise<SessionLogEntry[]> {
    ...
    logger.debug('[SESSION_SCANNER] Reading a session transcript');
    let file: string;
    try {
        file = await readFile(expectedSessionFile, 'utf-8');
    } catch {
        logger.debug('[SESSION_SCANNER] Session transcript not found');
        return [];
    }
    ...
    for (let l of lines) {
        try {
            ...
            logger.debug('[SESSION_SCANNER] Error processing a transcript message');
            continue;
        }
    }
    return entries;
