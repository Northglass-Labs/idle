...
export function claudeCheckSession(sessionId: string, path: string) {
    ...
    if (!sessionExists) {
        logger.debug('[claudeCheckSession] Session transcript not found');
        return false;
    }
    ...
    const hasGoodMessage = !!sessionData.find((v, index) => {
        ...
        try {
            ...
            logger.debug('[claudeCheckSession] Malformed transcript line', {
                lineNumber: index + 1,
            });
            return false;
        }
    ...
    logger.debug('[claudeCheckSession] Transcript validation complete', {
        valid: hasGoodMessage,
    });
