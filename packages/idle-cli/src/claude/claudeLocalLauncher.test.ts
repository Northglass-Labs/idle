...
describe('claudeLocalLauncher', () => {
    ...
    it('acknowledges a successful switch RPC with true', async () => {
        const localRun = createDeferred<void>();
        let switchHandler: ((params: { to: 'remote' }) => Promise<unknown>) | undefined;
        ...
        const session = {
            ...
            client: {
                ...
                closeClaudeSessionTurn: vi.fn(),
                rpcHandlerManager: {
                    registerHandler: vi.fn((method: string, handler: (params: { to: 'remote' }) => Promise<unknown>) => {
                        if (method === 'switch' && !switchHandler) {
        ...
        const launcher = claudeLocalLauncher(session as any);
        await vi.waitFor(() => expect(switchHandler).toBeDefined());

        await expect(switchHandler!({ to: 'remote' })).resolves.toBe(true);
        localRun.resolve();
        await expect(launcher).resolves.toEqual({ type: 'switch' });
    });
});
