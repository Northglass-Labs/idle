/**
 * Resolves the CLAUDE_CODE_ENTRYPOINT value Idle should use when launching
 ...
 * NOT itself an SDK entrypoint. Without an override, Idle-driven sessions can
 * therefore disappear from a plain `claude --resume` picker.
 *
 * Mitigation
 * ----------
 * Pre-set the env to a Claude-recognised entrypoint that is NOT in the SDK
 * filter set. "remote_mobile" matches Idle's role as a remote-mobile control
 * surface for Claude Code, is part of Claude Code's own allowlist, and keeps
 * Idle-touched sessions visible in `claude --resume`.
 *
 * If the caller already exported CLAUDE_CODE_ENTRYPOINT to something specific,
 * we respect that — the user/operator wins.
 */

export const IDLE_DEFAULT_ENTRYPOINT = 'remote_mobile' as const

export function resolveIdleEntrypoint(currentValue: string | undefined): string {
    if (currentValue && currentValue.length > 0) {
        return currentValue
    }
    return IDLE_DEFAULT_ENTRYPOINT
}
