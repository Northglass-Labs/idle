            ...
            if (message.type !== 'summary') {
                scannerMessageChain = scannerMessageChain.then(async () => {
                    try {
                        ...
                        logger.debug('[local]: failed to send Claude transcript message');
                    }
                });
            }
        }
        ...
        while (true) {
            ...
            try {
                await claudeLocal({
                    ...
                    // Push the verified sandbox state into the session metadata
                    // so the mobile UI's "(sandboxed)" badge reflects reality.
                    // applied=true → set metadata.sandbox to the active config.
                    // applied=false → clear it (init failed / Windows / disabled).
                ...
                logger.debug('[local]: launch error');
