...
export async function claudeRemoteLauncher(session: Session): Promise<'switch' | 'exit'> {
    ...
    async function doSwitch(): Promise<true> {
        ...
        if (message.type === 'assistant') {
            ...
            if (umessage.message.content && Array.isArray(umessage.message.content)) {
                for (let c of umessage.message.content) {
                    if (c.type === 'tool_use') {
                        logger.debug('[remote]: detected a tool use', {
                            hasParentToolCall: Boolean(umessage.parent_tool_use_id),
                        });
    ...
    try {
        ...
        while (!exitReason) {
            ...
            if (isNewSession) {
                ...
                logger.debug('[remote]: New session detected', {
                    hadPreviousSession: previousSessionId !== null,
                });
            } else {
                messageBuffer.addMessage('Continuing Claude session...', 'status');
                logger.debug('[remote]: Continuing existing session');
            }
            ...
            try {
                const remoteResult = await claudeRemote({
                    ...
                    onSandboxApplied: (applied) => {
                        session.client.updateMetadata((current) => ({
                            ...current,
                            sandbox: applied && session.sandboxConfig?.enabled ? session.sandboxConfig : null,
                        }));
                    },
                    ...
                    nextMessage: async () => {
                        if (pending) {
                            let p = pending;
                            pending = null;
                            session.client.setActiveRequestId(p.mode.requestId ?? null);
                        ...
                        if (msg) {
                            ...
                            modeHash = msg.hash;
                            mode = msg.mode;
                            session.client.setActiveRequestId(mode.requestId ?? null);
                            ...
                            if (attachments.length > 0) {
                                ...
                                for (const att of attachments) {
                                    ...
                                    if (!detected) {
                                        logger.debug('[remote] Skipping unsupported attachment without a magic-byte match', {
                                            bytes: att.data.byteLength,
                                            hasClaimedMimeType: Boolean(att.mimeType),
                                        });
                                        continue;
                                    }
                                    contentBlocks.push({
                    ...
                    onSDKMetadata: (metadata) => {
                        logger.debug('[remote] SDK metadata received', {
                            toolCount: metadata.tools?.length ?? 0,
                            commandCount: metadata.slashCommands?.length ?? 0,
                    ...
                    onCompletionEvent: (message: string) => {
                        logger.debug('[remote]: Completion event received');
                    ...
                    onReady: () => {
                        ...
                        session.client.setActiveRequestId(null);
                ...
                for (let [toolCallId, { parentToolCallId }] of ongoingToolCalls) {
                    ...
                    if (converted) {
                        logger.debug('[remote]: terminating an interrupted tool call', {
                            hasParentToolCall: parentToolCallId !== null,
                        });
        ...
        // Reset Terminal
        logger.debug('[remote]: cleanup begin', {
            hasExitReason: exitReason !== null,
            hasInk: Boolean(inkInstance),
        });
        if (inkInstance) {
            inkInstance.unmount();
        }
        logger.debug('[remote]: interface unmounted');
        ...
        logger.debug('[remote]: cleanup completed');
        messageBuffer.clear();
