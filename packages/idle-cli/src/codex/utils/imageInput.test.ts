...
vi.mock('@/configuration', () => ({
    configuration: { idleHomeDir: '/home/test/.idle' },
}));
...
async function makeTempDir(): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), 'idle-codex-image-input-'));
    tempDirs.push(dir);
    return dir;
}
...
describe('resolveCodexImageCacheDir', () => {
    it('uses the explicit cache root when provided', () => {
        expect(resolveCodexImageCacheDir({
            cacheRootDir: '/tmp/idle-cache',
            sessionId: 'session-1',
        })).toBe('/tmp/idle-cache/session-1');
    });

    it('defaults to Idle local state instead of arbitrary OS temp', () => {
        expect(resolveCodexImageCacheDir({
            sessionId: 'session-4',
        })).toBe('/home/test/.idle/codex-image-cache/session-4');
    });

    it('keeps malformed session ids inside the cache root', () => {
        const cacheRootDir = '/tmp/idle-cache';
