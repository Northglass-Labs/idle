...
export class CodexPermissionHandler extends BasePermissionHandler {
    ...
    private shouldAutoApprove(toolName: string): boolean {
        // IDs are correlation values, not authenticated tool identities. Only
        // the provider-reported tool name may cross the auto-approval boundary.
        return CodexPermissionHandler.ALWAYS_AUTO_APPROVE_NAMES.has(toolName);
    }
    ...
    async handleToolCall(
        ...
        if (this.shouldAutoApprove(toolName)) {
            logger.debug(`${this.getLogPrefix()} Auto-approved the title tool`);
