...
describe('buildCodexTurnPrompt', () => {
    it('prepends the Idle app system prompt before the first Codex user message', () => {
    ...
    it('does not inject the Idle app preamble on normal follow-up turns', () => {
        const prompt = buildCodexTurnPrompt({
            message: 'continue',
            mode: {
                appendSystemPrompt: '<options><option>Yes</option></options>',
    ...
    it('can re-inject the Idle app prompt without title instruction after a thread reset', () => {
        const prompt = buildCodexTurnPrompt({
            message: 'start fresh',
            mode: {
                appendSystemPrompt: '<options><option>Yes</option></options>',
...
describe('hashCodexEnhancedMode', () => {
    it('never batches prompts from different authenticated sender requests', () => {
        ...
        expect(hashCodexEnhancedMode({
            ...baseMode,
            requestId: 'sender-request-a',
        })).not.toBe(hashCodexEnhancedMode({
            ...baseMode,
            requestId: 'sender-request-b',
        }));
    });
