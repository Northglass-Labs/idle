...
import { assertSafeOpenClawGatewayUrl } from './openclawGateway';
import type {
...
export class OpenClawSocket {
  ...
  constructor(options: OpenClawSocketOptions) {
    this.options = {
      ...
      displayName: options.displayName ?? 'Idle CLI',
      log: options.log ?? (() => {}),
    };
  }
  ...
  connect(config: OpenClawGatewayConfig): void {
    const safeUrl = assertSafeOpenClawGatewayUrl(config.url);
    this.config = { ...config, url: safeUrl.toString() };
  ...
  private doConnect(): void {
    ...
    const url = assertSafeOpenClawGatewayUrl(this.config.url).toString();
    this.options.log('Connecting to OpenClaw gateway');

    try {
      this.ws = new WebSocket(url);
    } catch {
      this.options.log('Failed to create OpenClaw WebSocket');
    ...
    this.ws.on('error', () => {
      this.options.log('OpenClaw WebSocket transport error');
    ...
    this.ws.on('close', (code) => {
      this.options.log(`OpenClaw WebSocket closed with code ${code}`);
  ...
  private async sendConnect(): Promise<void> {
    ...
    try {
      ...
      this.options.log('Using stored OpenClaw device identity');

      const storedToken = await loadDeviceAuthToken(this.options.homeDir, this.config.url);
      ...
      if (result.auth?.deviceToken) {
        ...
        await storeDeviceAuthToken(this.options.homeDir, this.config.url, {
      ...
      this.options.log('Connected to OpenClaw gateway');
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : '';
      this.options.log('OpenClaw connection failed');
  ...
  private handleMessage(data: string): void {
    ...
    try {
      ...
      this.options.log('OpenClaw gateway sent invalid JSON');
      return;
    }
    ...
    if (frame.type === 'res') {
      ...
      if (frame.event === 'connect.challenge' && !this.connectSent) {
        ...
        this.options.log('Received OpenClaw authentication challenge');
