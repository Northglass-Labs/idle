 ...
 * The daemon spawns this as: `node dist/index.mjs openclaw --idle-starting-mode remote --started-by daemon`
 *
 * Connects to an OpenClaw gateway via WebSocket, translates the gateway protocol
 * to Idle's AgentMessage format, and forwards everything through the session pipeline.
 */
...
export function resolveGatewayConfig(opts: RunOpenClawOptions): OpenClawGatewayConfig {
  // Priority: CLI args > env vars > openclaw binary auto-detection
  const configuredUrl = opts.gatewayUrl ?? process.env.OPENCLAW_GATEWAY_URL;
  const explicitToken = opts.gatewayToken ?? process.env.OPENCLAW_GATEWAY_TOKEN ?? undefined;
  const explicitPassword = opts.gatewayPassword ?? process.env.OPENCLAW_GATEWAY_PASSWORD ?? undefined;
  const detectedUrl = !configuredUrl || (!explicitToken && !explicitPassword) ? queryGatewayUrl() : null;
  const rawUrl = configuredUrl ?? detectedUrl;
  ...
  const url = assertSafeOpenClawGatewayUrl(rawUrl).toString();

  let token = explicitToken;
  if (!token && detectedUrl && canUseAutoDetectedOpenClawToken(url, detectedUrl)) {
    token = queryGatewayToken() ?? undefined;
  }
...
export async function runOpenClaw(opts: RunOpenClawOptions): Promise<void> {
  ...
  log('OpenClaw gateway configuration resolved');
  ...
  if (response) {
    log('Idle session created for OpenClaw');
  }
  ...
  if (response) {
    try {
      ...
      logger.debug('[openclaw] Failed to report session to daemon');
    }
  }

  const sessionManager = new AcpSessionManager();
  const messageQueue = new MessageQueue2<{ requestId?: string }>((mode) => mode.requestId ?? 'local');
  ...
  const onBackendMessage = (msg: AgentMessage) => {
    if (verbose) {
      log(summarizeOpenClawBackendMessageForLog(msg));
    }
    ...
    if (msg.type === 'status' && (msg.status === 'error' || msg.status === 'stopped')) {
      log(`Backend entered ${msg.status} state`);
      shouldExit = true;
      messageQueue.close();
  ...
  async function handleAbort() {
    ...
    try {
      ...
      logger.debug('[openclaw] Abort failed');
    }
  ...
  try {
    ...
    log('Connected to OpenClaw gateway session');
    ...
    while (!shouldExit) {
      ...
      log(summarizeOpenClawPromptForLog(batch.message));
      inTurn = true;
      session.setActiveRequestId(batch.mode.requestId ?? null);
    ...
    try {
      ...
      logger.debug('[openclaw] Session close failed');
    }
  }
}
