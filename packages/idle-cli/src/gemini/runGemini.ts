...
export async function runGemini(opts: {
  ...
  logger.debug('[Gemini] Using the configured machine identity');
  ...
  // Start with no sandbox claim. The ACP backend reports enforcement only
  // after its OS wrapper has initialized and the Gemini spawn is prepared.
  ...
  if (response) {
    try {
      logger.debug('[START] Reporting Gemini session to daemon');
      ...
      if (result.error) {
        logger.debug('[START] Failed to report Gemini session to daemon (may not be running)');
      } else {
        logger.debug('[START] Reported Gemini session to daemon');
      }
    } catch {
      logger.debug('[START] Failed to report Gemini session to daemon (may not be running)');
    }
  ...
  session.onUserMessage((message) => {
    ...
    if (message.meta?.permissionMode) {
      ...
      if (validModes.includes(message.meta.permissionMode as PermissionMode)) {
        ...
        logger.debug('[Gemini] Permission mode updated from user message');
      } else {
        logger.debug('[Gemini] Invalid permission mode ignored');
      }
    } else {
      logger.debug('[Gemini] User message received without a permission-mode override');
    }
    ...
    if (message.meta?.hasOwnProperty('model')) {
      ...
      if (message.meta.model === null) {
        ...
        if (previousModel !== messageModel) {
          ...
          logger.debug('[Gemini] Model selection changed');
        }
      }
  ...
  const sendReady = () => {
    ...
    try {
      ...
      logger.debug('[Gemini] Failed to send ready push');
    }
  };
  ...
  async function handleAbort() {
    ...
    try {
      ...
      logger.debug('[Gemini] Error during abort');
    } finally {
  ...
  const handleKillSession = async () => {
    ...
    try {
      ...
      logger.debug('[Gemini] Error during session termination');
      process.exit(1);
    }
  };
  ...
  logger.debug('[gemini] Initial model selection resolved', {
    hasEnvironmentOverride: typeof process.env[GEMINI_MODEL_ENV] === 'string',
    hasDisplayedModel: typeof displayedModel === 'string',
  });
  ...
  const updateDisplayedModel = (model: string | undefined, saveToConfig: boolean = false) => {
    ...
    logger.debug('[gemini] Updating displayed model', {
      changed: oldModel !== model,
      saveToConfig,
    });
    ...
    if (hasTTY && oldModel !== model) {
      ...
      logger.debug('[gemini] Adding a model update message to the UI buffer');
  ...
  if (hasTTY) {
    ...
    logger.debug('[gemini] Sending the initial model selection to the UI');
    ...
    switch (msg.type) {
      ...
      case 'status':
        const statusDetail = msg.detail
          ...
          : '';
        logger.debug('[gemini] Provider status changed', {
          hasDetail: statusDetail.length > 0,
          detailBytes: Buffer.byteLength(statusDetail, 'utf8'),
        });

        // Log error status with details
        if (msg.status === 'error') {
          logger.debug('[gemini] Provider error status received', {
            hasDetail: statusDetail.length > 0,
          });
      ...
      case 'tool-call':
        ...
        logger.debug('[gemini] Tool call received', { isInvestigationTool });
      ...
      case 'tool-result':
        ...
        logger.debug('[gemini] Tool result received', { isError, resultSize });
        ...
        if (isError) {
          ...
          logger.debug(`[gemini] Tool call failed (detailLength=${errorMsg.length})`);
          messageBuffer.addMessage(`Error: ${errorMsg}`, 'status');
        } else {
          // Log summary for large results (like investigation tools)
          if (resultSize > 1000) {
            logger.debug('[gemini] Large tool result received', { resultSize });
          }
      ...
      case 'exec-approval-request':
        ...
        logger.debug('[gemini] Exec approval request received');
      ...
      case 'patch-apply-begin':
        ...
        logger.debug('[gemini] Patch apply began', { changeCount });
      ...
      case 'patch-apply-end':
        ...
        logger.debug('[gemini] Patch apply ended', { success: success === true });
      ...
      case 'event':
        ...
        if (msg.name === 'thinking') {
          ...
          if (thinkingText) {
            ...
            logger.debug(`[gemini] Thinking chunk received (length=${thinkingText.length})`);
  ...
  try {
    ...
    while (!shouldExit) {
      ...
      if (wasSessionCreated && currentModeHash && message.hash !== currentModeHash) {
        ...
        const backendResult = createGeminiBackend({
          ...
          model: modelToUse,
          sandboxConfig,
          onSandboxApplied: (applied) => {
            session.updateMetadata((current) => ({
              ...current,
              sandbox: applied && sandboxConfig?.enabled ? sandboxConfig : null,
            }));
          },
        });
        ...
        logger.debug('[gemini] Model change resolved', {
          hasRequestedModel: typeof modelToUse === 'string',
        });

        // Update conversation history with new model
        conversationHistory.setCurrentModel(actualModel);

        logger.debug('[gemini] Starting a new ACP session');
        const { sessionId } = await geminiBackend.startSession();
        acpSessionId = sessionId;
        logger.debug('[gemini] New ACP session started');

        // Update displayed model in UI (don't save to config - this is backend initialization)
        logger.debug('[gemini] Updating the displayed model after session start');
      ...
      try {
        if (first || !wasSessionCreated) {
          ...
          if (!geminiBackend) {
            ...
            // Use model from factory result (single source of truth - no duplicate resolution)
            const actualModel = backendResult.model;
            logger.debug('[gemini] Backend created with a resolved model');
            logger.debug('[gemini] Updating the displayed model after backend creation');
          ...
          if (!acpSessionId) {
            ...
            const { sessionId } = await geminiBackend.startSession();
            acpSessionId = sessionId;
            logger.debug('[gemini] ACP session started');
            wasSessionCreated = true;
            currentModeHash = message.hash;

            // Model info is already shown in status bar via updateDisplayedModel
            logger.debug('[gemini] Model selection displayed in UI');
          }
        }
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            if (isRetryable && attempt < MAX_RETRIES) {
              logger.debug('[gemini] Retryable provider error', {
                attempt,
        ...
        logger.debug('[gemini] Gemini session failed');
        ...
        if (accumulatedResponse.trim()) {
          ...
          if (options.length > 0) {
            ...
            logger.debug('[gemini] Parsed response options', { optionCount: options.length });
    ...
    try {
      ...
      logger.debug('[gemini]: Error while closing session');
    }
    ...
    try {
      ...
      logger.debug('[gemini]: Failed to release Idle MCP ownership');
    }
  }
}
