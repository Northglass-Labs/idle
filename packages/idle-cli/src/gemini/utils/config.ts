...
export function readGeminiLocalConfig(): GeminiLocalConfig {
      ...
      try {
        ...
        if (!model) {
          ...
          if (foundModel && typeof foundModel === 'string') {
            model = foundModel;
            logger.debug('[Gemini] Found configured model');
          }
        }
        ...
        if (!googleCloudProject) {
          ...
          if (foundProject && typeof foundProject === 'string') {
            ...
            logger.debug('[Gemini] Found configured Google Cloud project');
          }
        }
      } catch (error) {
        logger.debug('[Gemini] Failed to read config', errorLogMetadata(error));
      }
    }
  }
  ...
  if (!googleCloudProject) {
    ...
    if (envProject) {
      ...
      logger.debug('[Gemini] Found Google Cloud project from environment');
    }
  }

  return { model, googleCloudProject, googleCloudProjectEmail };
}
...
export function determineGeminiModel(
  ...
  if (explicitModel !== undefined) {
    ...
    logger.debug('[Gemini] Selecting model', {
      hasEnvironmentOverride: typeof envModel === 'string' && envModel.length > 0,
      hasLocalPreference: typeof localConfig.model === 'string' && localConfig.model.length > 0,
    });
    const model = envModel || localConfig.model || DEFAULT_GEMINI_MODEL;
    logger.debug('[Gemini] Selected model source', {
      source: envModel ? 'environment' : localConfig.model ? 'local-config' : 'default',
    });
    return model;
  }
}
...
export function saveGeminiModelToConfig(model: string): void {
  try {
    ...
    logger.debug('[Gemini] Saved model preference');
  } catch (error) {
    logger.debug('[Gemini] Failed to save model to config', errorLogMetadata(error));
...
export function saveGoogleCloudProjectToConfig(projectId: string, email?: string): void {
  try {
    ...
    // Write config back
    writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf-8');
    logger.debug('[Gemini] Saved Google Cloud project preference');
  } catch (error) {
    logger.debug('[Gemini] Failed to save Google Cloud Project to config', errorLogMetadata(error));
