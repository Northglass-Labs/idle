...
export class GeminiDiffProcessor {
    ...
    processFsEdit(path: string, description?: string, diff?: string): void {
        logger.debug('[GeminiDiffProcessor] Processing fs-edit', {
            hasDescription: typeof description === 'string' && description.length > 0,
            hasDiff: typeof diff === 'string' && diff.length > 0,
        });
    ...
    processToolResult(toolName: string, result: any, callId: string): void {
        ...
        if (result && typeof result === 'object') {
            ...
            if (diff && path) {
                logger.debug('[GeminiDiffProcessor] Found diff in tool result', {
                    hasPath: typeof path === 'string' && path.length > 0,
                    diffLength: typeof diff === 'string' ? diff.length : 0,
                });
    ...
    private processDiff(path: string, unifiedDiff: string, description?: string): void {
        ...
        if (previousDiff !== unifiedDiff) {
            logger.debug('[GeminiDiffProcessor] Unified diff changed; sending GeminiDiff tool call', {
                diffLength: unifiedDiff.length,
                trackedFileCount: this.previousDiffs.size,
            });
        ...
        logger.debug('[GeminiDiffProcessor] Updated stored diff', {
            trackedFileCount: this.previousDiffs.size,
        });
    }
