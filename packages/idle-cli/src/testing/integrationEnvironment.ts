...
export function applyEnvironmentToProcess(env: IntegrationEnvironment) {
    process.env.IDLE_SERVER_URL = `http://localhost:${env.serverPort}`;
    process.env.IDLE_WEBAPP_URL = `http://localhost:${env.expoPort}`;
    process.env.IDLE_HOME_DIR = join(env.envDir, 'cli', 'home');
    process.env.IDLE_PROJECT_DIR = env.projectPath;
