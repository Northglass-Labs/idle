...
describe('offline warning privacy', () => {
    beforeEach(() => {
        connectionState.reset(); // Reset singleton state between tests
    });

    it('prints categorical guidance without URLs or caller-provided detail', () => {
        const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        const privateUrl = 'https://private.example.test/path?token=opaque';
        const privateDetail = 'private filesystem and provider detail';
        ...
        } as Parameters<typeof connectionState.fail>[0]);

        const output = consoleSpy.mock.calls.flat().join('\n');
        expect(output).toContain('Idle server unreachable');
        expect(output).toContain('automatic reconnect');
