...
describe('expandEnvironmentVariables', () => {
    ...
    it('never copies environment names, resolved values, or defaults into diagnostics', () => {
        const result = expandEnvironmentVariables({
            FIRST: '${PRIVATE_NAME_SENTINEL}',
            SECOND: '${DEFAULT_NAME_SENTINEL:-default-value-sentinel}',
            THIRD: '${MISSING_NAME_SENTINEL}',
        }, {
            PRIVATE_NAME_SENTINEL: 'resolved-value-sentinel',
        });

        expect(result).toEqual({
            FIRST: 'resolved-value-sentinel',
            SECOND: 'default-value-sentinel',
            THIRD: '${MISSING_NAME_SENTINEL}',
        });
        const diagnostics = [
    ...
    it('should handle authentication token expansion pattern', () => {
        const envVars = {
            SERVICE_AUTH_TOKEN: '${LOCAL_SERVICE_TOKEN}'
        };
        const sourceEnv = {
            LOCAL_SERVICE_TOKEN: 'synthetic-token-value'
        };

        const result = expandEnvironmentVariables(envVars, sourceEnv);
        expect(result).toEqual({
            SERVICE_AUTH_TOKEN: 'synthetic-token-value'
        });
    });
    ...
    it('should handle a configured profile environment scenario', () => {
        const profileEnvVars = {
            SERVICE_AUTH_TOKEN: '${LOCAL_SERVICE_TOKEN}',
            SERVICE_BASE_URL: 'https://service.example.test',
            SECONDARY_KEY: '${LOCAL_SECONDARY_KEY}',
            CUSTOM_PATH: '/custom:${HOME}/bin'
        };
        const daemonEnv = {
            LOCAL_SERVICE_TOKEN: 'synthetic-service-token',
            LOCAL_SECONDARY_KEY: 'synthetic-secondary-key',
            HOME: '/Users/test'
        };

        const result = expandEnvironmentVariables(profileEnvVars, daemonEnv);
        expect(result).toEqual({
            SERVICE_AUTH_TOKEN: 'synthetic-service-token',
            SERVICE_BASE_URL: 'https://service.example.test',
            SECONDARY_KEY: 'synthetic-secondary-key',
