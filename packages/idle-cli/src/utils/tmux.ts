 ...
 * Ensures consistent tmux handling across idle-cli with proper session naming
 */
...
type TmuxSpawnFailureKind = 'unavailable' | 'create-window' | 'invalid-pane-pid' | 'unexpected';

class TmuxSpawnFailure extends Error {
    constructor(
        public readonly kind: TmuxSpawnFailureKind,
        message: string,
    ) {
        super(message);
        this.name = 'TmuxSpawnFailure';
    }
}
...
export class TmuxUtilities {
    ...
    public static readonly DEFAULT_SESSION_NAME = "idle";
    ...
    async executeWinOp(
        ...
        if (!tmuxCmd) {
            logger.debug('[TMUX] Unknown operation rejected');
            return false;
        }
    ...
    async spawnInTmux(
        ...
        try {
            ...
            if (!tmuxCheck) {
                throw new TmuxSpawnFailure('unavailable', 'tmux is unavailable');
            }

            // Handle session name resolution
            // - undefined: Use first existing session or create "idle"
            // - empty string: Use first existing session or create "idle"
            ...
            if (!sessionName) {
                ...
                if (listResult && listResult.returncode === 0 && listResult.stdout.trim()) {
                    ...
                    logger.debug('[TMUX] Resolved first existing session');
                } else {
                    // No sessions exist, create "idle"
                    sessionName = 'idle';
                    logger.debug('[TMUX] No existing sessions; using the default session');
                }
            }

            const windowName = options.windowName || `idle-${Date.now()}`;
            ...
            if (options.cwd) {
                const cwdPath = typeof options.cwd === 'string' ? options.cwd : fileURLToPath(options.cwd);
            ...
            if (env && Object.keys(env).length > 0) {
                ...
                for (const [key, value] of Object.entries(env)) {
                    // Skip undefined/null values without persisting the variable name.
                ...
                const skippedVariableCount = missingValueCount + invalidNameCount;
                if (skippedVariableCount > 0) {
                    logger.warn('[TMUX] Skipped invalid environment entries', {
                        missingValueCount,
                        invalidNameCount,
                    });
                }
                logger.debug('[TMUX] Prepared tmux window environment', {
                    providedVariableCount: Object.keys(env).length,
            ...
            if (!createResult || createResult.returncode !== 0) {
                throw new TmuxSpawnFailure('create-window', 'Failed to create tmux window');
            }

            // Extract the PID from the output
            const panePid = parseInt(createResult.stdout.trim());
            if (isNaN(panePid)) {
                throw new TmuxSpawnFailure('invalid-pane-pid', 'tmux did not return a valid pane process ID');
            }

            logger.debug('[TMUX] Spawned command in tmux', {
                argumentCount: args.length,
                hasCustomSession: options.sessionName !== undefined && options.sessionName !== '',
                hasCustomWindow: options.windowName !== undefined,
                hasCustomWorkingDirectory: options.cwd !== undefined,
            });
            ...
            const failure = error instanceof TmuxSpawnFailure
                ? error
                : new TmuxSpawnFailure('unexpected', 'Failed to spawn process in tmux');
            logger.debug('[TMUX] Failed to spawn process', { failureKind: failure.kind });
            return {
                success: false,
    ...
    async getSessionInfoFromString(sessionIdentifier: string): Promise<TmuxSessionInfo | null> {
        try {
            ...
            if (error instanceof TmuxSessionIdentifierError) {
                logger.debug('[TMUX] Invalid session identifier rejected');
            } else {
                logger.debug('[TMUX] Failed to get session information');
            }
            return null;
        }
    }
    ...
    async killWindow(sessionIdentifier: string): Promise<boolean> {
        try {
            ...
            if (error instanceof TmuxSessionIdentifierError) {
                logger.debug('[TMUX] Invalid window identifier rejected');
            } else {
                logger.debug('[TMUX] Failed to kill window');
            }
            return false;
        }
    }
