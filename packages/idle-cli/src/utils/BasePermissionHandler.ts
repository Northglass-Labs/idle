    ...
    protected setupRpcHandler(): void {
        this.session.rpcHandlerManager.registerHandler<PermissionResponse, void>(
            ...
            async (response) => {
                ...
                logger.debug(`${this.getLogPrefix()} Permission request ${response.approved ? 'approved' : 'denied'}`);
            }
        );
    }
    ...
    abortAll(): void {
        ...
        for (const [, pending] of pendingSnapshot) {
            try {
                ...
                logger.debug(`${this.getLogPrefix()} Error resolving aborted permission request`);
            }
        }
        ...
        try {
            ...
            for (const [, pending] of pendingSnapshot) {
                try {
                    ...
                    logger.debug(`${this.getLogPrefix()} Error rejecting pending permission request`);
                }
            }
