...
/**
 * Bundles idle-server into a self-contained artifact directory.
 *
 * Uses `bun build --compile` to produce a single platform-specific binary, then copies the
 * pglite WASM/data files and Prisma migrations alongside. The CLI does not depend on the
 * server workspace package — the CLI reaches into the sibling directory only at build time.
 *
 * Layout produced:
 *   <out-dir>/
 *     <platform>/
 *       idle-server                   # bun-compiled binary
...
const SERVER_DIR = path.resolve(REPO_ROOT, 'packages/idle-server');
...
function platformBinaryName(plat) {
    return plat.endsWith('-win32') ? 'idle-server.exe' : 'idle-server';
}
...
function buildPlatform(plat, outRoot) {
    ...
    console.log(`\n→ Bundling idle-server for ${plat} (${target})`);
    run(
        'bun',
        [
            'build',
...
function main() {
    ...
    console.log(`\n✓ idle-server bundle written to ${outDir}`);
}
