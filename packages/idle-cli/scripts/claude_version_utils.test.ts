...
describe('Claude Version Utils - Cross-Platform Detection', () => {
  ...
  describe('/goal hook JSON validation warning', () => {
    it('uses Idle branding in the operator-facing recovery guidance', () => {
      const warning = formatGoalHookJsonValidationWarning('2.1.177');

      expect(warning).toContain('use Idle remote mode');
      expect(warning).not.toMatch(/Happ[y]/);
    });
...
describe('IDLE_CLAUDE_PATH env var', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.IDLE_CLAUDE_PATH;
  });

  it('should use IDLE_CLAUDE_PATH when set', () => {
    process.env.IDLE_CLAUDE_PATH = testClaudePath;
    const result = findGlobalClaudeCliPath();
    expect(result?.source).toBe('IDLE_CLAUDE_PATH');
  ...
  it('should fall back to auto-discovery when env var not set', () => {
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('IDLE_CLAUDE_PATH');
  });

  it('should ignore env var if path does not exist', () => {
    process.env.IDLE_CLAUDE_PATH = '/nonexistent/path/claude';
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('IDLE_CLAUDE_PATH');
  });
});

describe('Bun native binary detection', () => {
  const originalPlatform = process.platform;
  ...
  it('findClaudeInPath returns the resolved Bun native binary on Unix', () => {
    Object.defineProperty(process, 'platform', { value: 'darwin', configurable: true });

    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'idle-bun-path-'));
    const shimPath = path.join(tempRoot, '.bun', 'bin', 'claude');
    ...
    expect(result).toEqual({ path: fs.realpathSync(binaryPath), source: 'Bun' });
  });

  it('findBunGlobalCliPath resolves Bun native binaries from ~/.bun/bin/claude', () => {
    Object.defineProperty(process, 'platform', { value: 'darwin', configurable: true });

    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'idle-bun-global-'));
    const fakeHome = path.join(tempRoot, 'home');
    const shimPath = path.join(fakeHome, '.bun', 'bin', 'claude');
    const binaryPath = path.join(
      fakeHome,
      '.bun',
      'install',
    ...
    __setTestOverrides({
      execSync: (command: string) => {
        if (command === 'which bun') {
          return `${path.join(fakeHome, '.bun', 'bin', 'bun')}\n`;
        }
        throw new Error(`Unexpected command: ${command}`);
      },
      homedir: () => fakeHome
    ...
    expect(findBunGlobalCliPath()).toBe(fs.realpathSync(binaryPath));
    expect(getVersion(binaryPath)).toBe('2.1.113');
  });
});
