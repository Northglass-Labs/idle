...
/**
 * Runs `expo export -p web` in packages/idle-app and copies the output into
 * a package-owned artifact directory. By default this writes to idle-cli/tools/webapp
 * for local development. Release packaging can pass --out-dir to place it elsewhere.
 *
 * idle-cli does NOT depend on idle-app — we reach into the sibling at build time only.
 */
...
const APP_DIR = path.resolve(REPO_ROOT, 'packages/idle-app');
...
function main() {
    ...
    console.log(`→ Building idle-app web bundle (expo export)`);
    rmrf(APP_DIST);
    run('yarn', ['expo', 'export', '-p', 'web', '--output-dir', 'dist'], { cwd: APP_DIR });
