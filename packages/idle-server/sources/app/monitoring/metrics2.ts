...
register.setDefaultLabels({ app: 'idle-server' });

const KNOWN_CLIENT_FAMILIES = new Set([
    'cli-coding-session',
    'cli-daemon',
    'cli-control-plane',
    'ios',
...
function parseClientLabels(raw: unknown): ClientLabels {
    if (typeof raw !== 'string' || raw.length > 128) {
        return { client: 'unknown', client_type: 'unknown' };
    }
    const family = raw.split('/', 1)[0]?.toLowerCase();
    if (!family || !KNOWN_CLIENT_FAMILIES.has(family)) {
        return { client: 'unknown', client_type: 'unknown' };
    }
    // Never include the raw header or version. Both are client-controlled and
    // would let an attacker create unbounded Prometheus label cardinality.
    return { client: family, client_type: family };
}
