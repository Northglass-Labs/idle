...
import type { SessionMessageContent } from "@northglass/idle-wire";
import { pendingSocketAdmissions } from '@/app/api/socket/pendingSocketAdmissions';

const CANCEL_PENDING_ADMISSIONS_EVENT = 'idle:cancel-pending-socket-admissions';

interface EventRouterOptions {
    distributePendingAdmissionCancellation?: boolean;
}
...
export interface SessionScopedConnection {
    ...
    accountAuthorizationGeneration: number;
    isAuthorizationCurrent: () => Promise<boolean>;
    rpcRegistrationAuthorized?: boolean;
}
...
class EventRouter {
    private io!: Server;
    private distributePendingAdmissionCancellation = false;
    private pendingCancellationListener?: (userId: unknown, acknowledge?: (response: unknown) => void) => void;

    // === INITIALIZATION ===

    init(io: Server, options: EventRouterOptions = {}): void {
        if (this.pendingCancellationListener) {
            (this.io as any)?.off?.(CANCEL_PENDING_ADMISSIONS_EVENT, this.pendingCancellationListener);
        }
        this.io = io;
        this.distributePendingAdmissionCancellation = options.distributePendingAdmissionCancellation === true;
        this.pendingCancellationListener = (userId, acknowledge) => {
            if (typeof userId !== 'string' || userId.length === 0 || userId.length > 128) {
                acknowledge?.({ ok: false });
                return;
            }
            pendingSocketAdmissions.cancelUser(userId);
            acknowledge?.({ ok: true });
        };
        (this.io as any).on?.(CANCEL_PENDING_ADMISSIONS_EVENT, this.pendingCancellationListener);
    }
    ...
    /**
     * Immediately terminate every live socket for an account. Room operations
     * are propagated by Socket.IO's configured adapter, so an operator
     * suspension also reaches connections owned by another relay process.
     */
    async disconnectUserConnections(userId: string): Promise<void> {
        pendingSocketAdmissions.cancelUser(userId);
        let pendingCancellationError: unknown;
        if (this.distributePendingAdmissionCancellation) {
            try {
                const responses = await (this.io as any).serverSideEmitWithAck(
                    CANCEL_PENDING_ADMISSIONS_EVENT,
                    userId,
                ) as unknown[];
                if (!responses.every((response) => (
                    ...
                    throw new Error('A relay rejected pending admission cancellation');
                }
            } catch (error) {
        ...
        // Sweep every established socket even if one relay failed to
        // acknowledge pending-state cancellation. The account remains durably
        // suspended and the caller still receives the distributed failure.
        await this.disconnectRoom(`user:${userId}`);
        if (pendingCancellationError) throw pendingCancellationError;
    }

    /** Revoke every live capability derived from one session. */
    async disconnectSessionConnections(userId: string, sessionId: string): Promise<void> {
        await this.disconnectRoom(`user:${userId}:session:${sessionId}`);
    }

    /** Revoke every live capability derived from one machine. */
    async disconnectMachineConnections(userId: string, machineId: string): Promise<void> {
        await this.disconnectRoom(`user:${userId}:machine:${machineId}`);
    }

    private async disconnectRoom(room: string): Promise<void> {
        // BroadcastOperator.disconnectSockets() intentionally returns void,
        // even when a cluster adapter publishes asynchronously. Invoke the
        // adapter directly so deletion/suspension does not report completion
        // before the revocation message has been durably published.
        await this.io.sockets.adapter.disconnectSockets({
            rooms: new Set([room]),
            except: new Set(),
            flags: {},
        }, true);
...
export function buildUpdateSessionUpdate(sessionId: string, updateSeq: number, updateId: string, metadata?: { value: string; version: number }, agentState?: { value: string | null; version: number }): UpdatePayload {
    return {
        id: updateId,
        seq: updateSeq,
        body: {
...
export function buildDeleteSessionUpdate(sessionId: string, updateSeq: number, updateId: string, recordCreatedAt?: Date): UpdatePayload {
    return {
        id: updateId,
        seq: updateSeq,
        body: {
            t: 'delete-session',
            sid: sessionId,
            ...(recordCreatedAt && { recordCreatedAt: recordCreatedAt.getTime() }),
        },
        createdAt: Date.now()
    };
}
...
export function buildDeleteMachineUpdate(machineId: string, updateSeq: number, updateId: string, recordCreatedAt?: Date): UpdatePayload {
    return {
        id: updateId,
        seq: updateSeq,
        body: {
...
export function buildDeleteArtifactUpdate(artifactId: string, updateSeq: number, updateId: string, recordCreatedAt?: Date): UpdatePayload {
    return {
        id: updateId,
        seq: updateSeq,
        body: {
