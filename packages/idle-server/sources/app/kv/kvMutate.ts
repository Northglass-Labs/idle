...
import { canAllocateKVRows } from "@/app/limits/persistedResourceQuotas";

export class KVQuotaExceededError extends Error {
    constructor() {
        super('KV storage quota exceeded');
...
export async function kvMutate(
    ...
    return await inTx(async (tx) => {
        ...
        for (const mutation of mutations) {
            ...
            if (!existing && mutation.version === -1) {
                newRows += 1;
            }
        ...
        if (!await canAllocateKVRows(tx, ctx.uid, newRows)) {
            throw new KVQuotaExceededError();
        }
        ...
        // Reserve the sequence in the mutation transaction so an allocation
        // failure rolls back the KV write instead of silently losing its sync
        // event after commit.
        const updateSeq = await allocateUserSeq(ctx.uid, tx);
        const updatePayload = buildKVBatchUpdateUpdate(changes, updateSeq, randomKeyNaked(12));

        // Send a single bundled notification after commit.
        afterTx(tx, () => {
