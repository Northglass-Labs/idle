...
import { MAX_KV_LIST_ITEMS, kvList } from "@/app/kv/kvList";
import { kvBulkGet } from "@/app/kv/kvBulkGet";
import { KVQuotaExceededError, kvMutate } from "@/app/kv/kvMutate";
import { log } from "@/utils/log";
import { TagSchema, EncryptedBlobSchema } from "@/app/api/routes/_schemas";
...
export function kvRoutes(app: Fastify) {
    ...
    app.get('/v1/kv/:key', {
        ...
        try {
            ...
            log({ module: 'api', level: 'error' }, 'Failed to get KV value');
    ...
    app.get('/v1/kv', {
        ...
        schema: {
            querystring: z.object({
                prefix: TagSchema.optional(),
                limit: z.coerce.number().int().min(1).max(MAX_KV_LIST_ITEMS).default(MAX_KV_LIST_ITEMS)
            }),
            response: {
                200: z.object({
        ...
        try {
            ...
            log({ module: 'api', level: 'error' }, 'Failed to list KV items');
    ...
    app.post('/v1/kv/bulk', {
        ...
        schema: {
            body: z.object({
                keys: z.array(TagSchema).min(1).max(100)
            }),
            response: {
                200: z.object({
        ...
        try {
            ...
            log({ module: 'api', level: 'error' }, 'Failed to bulk get KV values');
    ...
    app.post('/v1/kv', {
        ...
        schema: {
            ...
            response: {
                ...
                429: z.object({
                    error: z.literal('KV storage quota exceeded')
                }),
                500: z.object({
        ...
        try {
            ...
            if (error instanceof KVQuotaExceededError) {
                return reply.code(429).send({ error: 'KV storage quota exceeded' });
            }
            log({ module: 'api', level: 'error' }, 'Failed to mutate KV values');
