...
import { getOperationalRecentPaths } from '@/sync/newSessionOperationalInputs';
...
export default function MachineDetailScreen() {
    ...
    const machineSessions = useMemo(() => {
        ...
        return sessions.filter(item => {
            ...
            return getOperationalSessionMetadata(session.metadata)?.machineId === machineId;
        }) as Session[];
    }, [sessions, machineId]);
    ...
    const recentPaths = useMemo(() => {
        return getOperationalRecentPaths(machineSessions, machineId ?? '');
    }, [machineSessions]);
        ...
        try {
            ...
            switch (result.type) {
                ...
                case 'requestToApproveCodexNativeSandbox':
                    Modal.alert(
                        t('common.error'),
                        'Choose Codex from the New Session screen to review its sandbox options.',
                    );
                    break;
                case 'error':
    ...
    const pastUsedRelativePath = useCallback((session: Session) => {
        ...
        if (!metadata) return 'unknown path';
        return formatPathRelativeToHome(metadata.path, metadata.homeDir);
    }, []);
    ...
    return (
        <>
            ...
            >
                ...
                {/* Bounded navigation history for this machine. */}
                {previousSessions.length > 0 && (
                    <ItemGroup title="Recent Sessions">
