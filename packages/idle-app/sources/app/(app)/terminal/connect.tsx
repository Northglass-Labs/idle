...
export default function TerminalConnectScreen() {
    ...
    const { requestTerminalPairing, isLoading } = useConnectTerminal({
        onSuccess: () => {
            router.back();
        }
    });
    ...
    const handleConnect = async () => {
        if (publicKey) {
            // Convert the hash key format to the expected idle:// URL format
            const authUrl = `idle://terminal?${publicKey}`;
            await requestTerminalPairing(authUrl);
        }
    };
        ...
        <ItemList>
            ...
            <ItemGroup title={t('terminal.connectionDetails')}>
                <Item
                    ...
                    icon={<Ionicons name="key-outline" size={29} color="#32D74B" />}
                    showChevron={false}
                />
                <Item
