...
import { decodeBase64FileContent, FILE_LOAD_LIMITS } from '@/sync/fileLoadPolicy';
...
export default React.memo(function FileScreen() {
    ...
    const sessionPath = operationalMetadata?.path ?? null;
    let rawPath = '';
    ...
    React.useEffect(() => {
        ...
        const loadFile = async () => {
            try {
                ...
                if (!sessionPath || !resolvedPath?.withinSessionRoot) {
                    if (!isCancelled) {
                        setError('File is outside the authenticated session root.');
                        setIsLoading(false);
                    }
                    return;
                }
                ...
                if (sessionPath && sessionId && gitDiffPath && gitDiffPath !== '.') {
                    try {
                        const diffResponse = await sessionGitDiff(sessionId, {
                            ...
                            mode: 'working',
                            timeout: 5000,
                            maxBytes: FILE_LOAD_LIMITS.explicitOpen.maxBytesPerResponse,
                        });
                        ...
                        console.log('Could not fetch git diff');
                    }
                }
                ...
                if (!isCancelled) {
                    if (response.success && response.content) {
                        ...
                        const content = decoded.isBinary ? '' : decoded.text;
                        setFileContent({ content, encoding: 'utf8', isBinary: decoded.isBinary });
                        storage.getState().applyFileCache(sessionId!, filePath, content, fetchedDiff, decoded.isBinary);
                    } else {
