...
import { readBoundedTextResponse } from '@/sync/boundedJsonResponse';
import { streamingFetch } from '@/sync/streamingFetch';

const MAX_SERVER_PROBE_RESPONSE_BYTES = 64 * 1024;
...
export default function ServerConfigScreen() {
    ...
    const validateServer = async (url: string): Promise<boolean> => {
        try {
            ...
            const text = await readBoundedTextResponse(response, MAX_SERVER_PROBE_RESPONSE_BYTES);
            if (!text.includes('Welcome to Idle Server!')) {
                setError(t('server.notValidIdleServer'));
                return false;
            }
    ...
    const handleSave = async () => {
        ...
        const validation = validateServerUrl(normalizedUrl);
        if (!validation.valid) {
        ...
        if (confirmed) {
            try {
                await auth.switchServer(normalizedUrl);
            } catch {
                setError(t('server.failedToConnectToServer'));
            }
        }
