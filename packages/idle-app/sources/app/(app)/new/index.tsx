...
} from '@/sync/newSessionOperationalInputs';
...
function NewSessionScreen() {
    ...
    const pathItems = React.useMemo<PickerItem[]>(() => {
        ...
        const paths = getOperationalRecentPaths(sessions, selectedMachineId);
    ...
    React.useEffect(() => {
        ...
        loadNewSessionWorktrees({
            ...
            resolvedPath: debouncedResolvedSelectedPath,
            machineOnline: true,
        }, listWorktrees).then(worktrees => {
            if (cancelled) return;
    ...
    const handleSend = React.useCallback(async (
        ...
        codexProviderNativeSandboxApproved: boolean = false,
    ) => {
        ...
        try {
            ...
            // The MMKV-backed useNewSessionDraft hook is the single
            // "remember last choice" mechanism for these fields. It
            // persists synchronously on every change; sync settings do not.
            // Schema fields retained in sync/settings.ts for back-compat.
            ...
            const result = await machineSpawnNewSession({
                ...
                // Pass the user's Co-Authored-By preference into the session.
                commitAttribution: storage.getState().localSettings.commitAttribution,
            });
            ...
            switch (result.type) {
                ...
                case 'requestToApproveDirectoryCreation': {
                    ...
                    if (approved) {
                        await handleSend(true, codexProviderNativeSandboxApproved);
                    }
                    break;
                }
                case 'requestToApproveCodexNativeSandbox': {
                    const approved = await Modal.confirm(
                        'Use Codex Native Sandbox?',
                        'Your Codex ChatGPT sign-in is stored in the macOS keychain and cannot be delegated to Idle\'s additional isolation layer. Continue with Codex\'s built-in workspace sandbox and normal approval prompts? Idle\'s extra read and network restrictions will not apply.',
                        { cancelText: t('common.cancel'), confirmText: t('common.continue') },
                    );
                    if (approved) {
                        await handleSend(approvedNewDirectoryCreation, true);
                    }
                    break;
                }
                case 'error':
                    ...
                    <>
                        <View style={styles.configRowWithToggle}>
                            <Pressable
                                ...
                                accessibilityRole="button"
                                accessibilityLabel={`Select machine: ${machineName}`}
                            >
