...
export default React.memo(() => {
    ...
    const loadPushSettings = useCallback(async (showError = false) => {
        ...
        try {
            ...
            console.error('Failed to load push notification settings');
            if (showError) {
    ...
    const handleLogout = async () => {
        ...
        if (confirmed) {
            try {
                ...
                Modal.alert(t('common.error'), `${t('errors.networkError')}. ${t('errors.tryAgain')}.`);
            }
        }
    };

    // Permanently delete the account (App Store Guideline 5.1.1(v)). Strong,
    // single confirmation, then wipe server-side data and clear local creds.
    const [deletingAccount, handleDeleteAccount] = useIdleAction(async () => {
        if (!auth.credentials) {
            return;
        }
        const confirmed = await Modal.confirm(
            ...
            { confirmText: t('settingsAccount.deleteAccountConfirmButton'), destructive: true }
        );
        if (!confirmed) {
            return;
        }
    ...
    const handlePushPermissionRequest = useCallback(async () => {
        ...
        try {
            ...
            if (result.openedSettings) {
                Modal.alert('Open Settings', 'The system will not show the permission prompt again, so Idle opened Settings instead.');
                return;
            }
    ...
    const handleDeletePushToken = useCallback(async (pushToken: PushToken) => {
        ...
        try {
            ...
            console.error('Failed to delete push token');
            ...
            <ItemList>
                ...
                <ItemGroup title={t('settingsAccount.accountInformation')}>
                    ...
                    {Platform.OS !== 'web' && (
                        <Item
                            ...
                            longDescription="Scan a QR code from another logged-in Idle device or the idle CLI to add a new device to your account. Sessions and settings sync across paired devices. Your encryption keys never leave the device — they're shared via the QR pairing handshake."
                            icon={<Ionicons name="qr-code-outline" size={29} color="#32D74B" />}
                ...
                >
                    <Item
                        ...
                        icon={<Ionicons name="notifications-outline" size={29} color="#32D74B" />}
