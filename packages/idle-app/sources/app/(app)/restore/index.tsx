        ...
        const startQRAuth = async () => {
            try {
                ...
                if (credentials && !isCancelledRef.current) {
                    const enteredCode = await Modal.prompt(
                        'Verify account pairing',
                        'Enter the verification code shown on the device that approved this link. If no code appeared there, cancel this pairing.',
                        {
                    ...
                    if (enteredCode?.trim().toUpperCase() !== credentials.verificationCode) {
                        Modal.alert(
                            'Pairing not verified',
                            'The verification code did not match. No credentials were saved. Start again with a new QR code.',
                        );
                        router.back();
                        return;
                    }
                ...
                if (!isCancelledRef.current) {
                    console.error('QR authentication failed');
                    Modal.alert(t('common.error'), t('errors.authenticationFailed'));
                }
            } finally {
                if (!isCancelledRef.current) {
            ...
            <View style={styles.container}>
                ...
                <View style={{justifyContent: 'flex-end' }}>
                    <Text style={styles.secondInstructionText}>
                        {t('components.restoreScreen.step1')}{'\n'}
                        {t('components.restoreScreen.step2')}{'\n'}
                        {t('components.restoreScreen.step3')}{'\n'}
                        {t('components.restoreScreen.step4')}
                    </Text>
                </View>
                {!authReady && (
                ...
                {authReady && (
                    <QRCode
                        data={'idle:///account?' + encodeBase64(keypair.publicKey, 'base64url')}
                        size={300}
