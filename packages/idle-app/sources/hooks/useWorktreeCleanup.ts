...
export async function maybeCleanupWorktree(
    ...
    const selectedMetadata = getOperationalSessionMetadata(
        state.sessions[sessionId]?.metadata,
    );
    if (
        selectedMetadata?.machineId !== machineId
        || selectedMetadata.path !== sessionPath
    ) {
        return;
    }

    // 1. Check if other active sessions use the same worktree
    const allSessions = state.getActiveSessions();
    const otherOnSameWorktree = allSessions.some(
        (session) => {
            ...
            return metadata?.machineId === machineId && metadata.path === sessionPath;
        },
    );
