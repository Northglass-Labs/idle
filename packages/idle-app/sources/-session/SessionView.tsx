...
import { machineResumeSession, sessionAbort, sessionGoalAction } from '@/sync/ops';
import { getOperationalSessionMetadata, storage, useIsDataReady, useLocalSetting, useRealtimeStatus, useSessionMessages, useSessionUsage, useSetting } from '@/sync/storage';
...
export const SessionView = React.memo((props: { id: string }) => {
    ...
    const mainContent = (
        <>
            ...
            <View testID="session-screen" style={{ flex: 1, paddingTop: !(isLandscape && deviceType === 'phone' && Platform.OS !== 'web') ? safeArea.top + headerHeight + (!isTablet && realtimeStatus !== 'disconnected' ? 32 : 0) : 0 }}>
                {!isDataReady ? (
    ...
    // When a sidebar file is selected, the file diff overlays the main content.
...
type ChatComposerHandle = {
    getMessage: () => string;
    clearMessage: () => void;
    // send-to-resume: restore a message into the composer (send-to-resume failure path).
...
function SessionViewLoaded({ sessionId, session }: { sessionId: string, session: Session }) {
    ...
    const cliVersion = operationalMetadata?.version;
    ...
    const availableModels = React.useMemo(() => (
        getAvailableModels(flavor, operationalMetadata, t)
    ), [flavor, operationalMetadata]);
    const availableModes = React.useMemo(() => (
        getAvailablePermissionModes(flavor, operationalMetadata, t)
    ), [flavor, operationalMetadata]);
    ...
    const permissionMode = React.useMemo<PermissionMode | null>(() => (
        resolveCurrentOption(availableModes, [
            ...
            operationalMetadata?.currentOperatingModeCode,
        ])
    ), [availableModes, session.permissionMode, effectiveAgentDefaults.permissionMode, operationalMetadata?.currentOperatingModeCode]);

    const modelMode = React.useMemo<ModelMode | null>(() => (
        resolveCurrentOption(availableModels, [
            session.modelMode,
            effectiveAgentDefaults.modelMode,
            operationalMetadata?.currentModelCode,
        ])
    ), [availableModels, session.modelMode, effectiveAgentDefaults.modelMode, operationalMetadata?.currentModelCode]);
    ...
    const fileViewerEnabled = useSetting('fileViewerEnabled');
    ...
    // send-to-resume: send-to-resume from the composer.
    // When the session is disconnected AND it's resumable, hitting send wakes
    // the session and delivers the message in one action. Bypasses the
    // discrete Resume button for the common "I want to keep talking" case.
    //
    // Why not use the existing useSessionQuickActions resume callback: it's
    // wrapped in useIdleAction which doesn't expose a promise — we need to
    // know when the resume RPC completes so the message lands in the woken
    // Claude process, not before it's listening. Call machineResumeSession
    // directly here and await it.
    const [sendingToResume, setSendingToResume] = React.useState(false);
    const handleSendToResume = React.useCallback(async (text: string, attachments?: typeof selectedImages) => {
        if (!machineId) return;
        setSendingToResume(true);
        try {
            const result = await machineResumeSession({ machineId, sessionId });
            if (result.type === 'success') {
                sync.sendMessage(result.sessionId, text, { source: 'chat', attachments });
            } else {
                // Resume failed — put the message back so the user sees it
                // and can decide what to do (retry, copy out, etc.).
                composerHandleRef.current?.setMessage(text);
            }
        } catch {
            composerHandleRef.current?.setMessage(text);
        } finally {
    ...
    const handleSend = React.useCallback(() => {
        ...
        if (liveMessage.trim() || (expImageUpload && selectedImages.length > 0)) {
            ...
            if (isDisconnected && canResume) {
                // send-to-resume send-to-resume: wake the session, then deliver.
                void handleSendToResume(liveMessage, attachments);
            } else {
                sync.sendMessage(sessionId, liveMessage, { source: 'chat', attachments });
            }
        }
    }, [sessionId, expImageUpload, selectedImages, clearImages, isDisconnected, canResume, handleSendToResume]);
    ...
    const handleGoalAction = React.useCallback(async (action: AgentGoalAction) => {
        await performAgentGoalAction({
            ...
            onError: () => console.error('Failed to perform goal action'),
        });
    ...
    const handleMicrophonePress = React.useCallback(async () => {
        ...
        if (realtimeStatus === 'disconnected' || realtimeStatus === 'error') {
            try {
                ...
                const started = await startRealtimeSession(sessionId, initialPrompt);
                if (started) {
                    const hasPro = storage.getState().purchases.entitlements['pro'] ?? false;
                    trackVoiceSessionStarted({
                        hasPro,
                        onboardingPromptLoads: getVoiceOnboardingPromptLoadCount(),
                        voiceMessages: getVoiceMessageCount(),
                    });
                }
            } catch (error) {
    ...
    // Keepalive polling for the visible session defends against a silent-stale
    // state where the socket
    // stays "connected", app stays foreground, but the live `update` stream
    // goes silent until the user sends something (which forces a REST
    // round-trip that piggybacks queued events). The cheapest mitigation is
    // a periodic forward-since-last-seq fetch — it's a no-op when nothing's
    // new and surfaces missed messages immediately when there's drift.
    //
    // 30 seconds is a balance: tight enough that a user re-reading a long
    // pasted message will catch up within a window they won't notice;
    // loose enough that an idle session burns negligible bandwidth. Polling
    // is gated on AppState 'active' so a backgrounded tab doesn't tick.
    React.useEffect(() => {
        const POLL_MS = 30000;
    ...
    const composer = (
        <ChatComposer
            ...
            // send-to-resume placeholder: when the session is dormant but we CAN
            // wake it, surface the affordance in the composer itself rather
            // than relying only on the discrete Resume button above.
            placeholder={isDisconnected && canResume
                ? t('session.inputPlaceholderResume')
            ...
            // send-to-resume: allow send on disconnected sessions when resume is
            // available — the send handler routes through send-to-resume.
            // Sessions that aren't resumable (missing machine / offline /
            // idle-agent not paired) keep the old blockSend behavior.
            blockSend={(isDisconnected && !canResume) || sendingToResume || resumingSession}
            onSend={handleSend}
            ...
            onFileViewerPress={fileViewerEnabled && !isTablet ? handleFileViewerPress : undefined}
    ...
    // copy-this-command hint when the toggle is off or the machine isn't
    // reachable.
    ...
    const input = (
        <>
            ...
            {/* The most recent failed send is recoverable with inline Retry or Discard. */}
            <CenteredInputWidth horizontalPadding={sessionInputHorizontalPadding}>
                <FailedMessageBanner sessionId={sessionId} />
            </CenteredInputWidth>
            {composer}
        </>
    );
