...
import { clearRegisteredPushToken, loadRegisteredPushToken, saveRegisteredPushToken } from './pushTokenStorage';
...
export async function getCurrentExpoPushToken(): Promise<string | null> {
    ...
    if (!permission.granted) {
        return await loadRegisteredPushToken();
    }
    ...
    try {
        ...
        return tokenData.data ?? await loadRegisteredPushToken();
    } catch (error) {
...
export async function syncCurrentPushToken(credentials: AuthCredentials): Promise<SyncCurrentPushTokenResult> {
    ...
    if (!permission.granted) {
        if (!permission.canAskAgain) {
            return {
                registered: false,
                token: await loadRegisteredPushToken(),
                permission,
            };
        }
    ...
    const previousToken = await loadRegisteredPushToken();

    await registerPushToken(credentials, currentToken);
    await saveRegisteredPushToken(currentToken);

    if (previousToken && previousToken !== currentToken) {
        try {
            await unregisterPushToken(credentials, previousToken);
        } catch (error) {
            console.log('Failed to unregister previous push token');
        }
    }
...
export async function removePushToken(credentials: AuthCredentials, token: string): Promise<void> {
    ...
    if (await loadRegisteredPushToken() === token) {
...
export function getCurrentPushDeviceMetadata(): CurrentPushDeviceMetadata {
    ...
    const appParts = [
        Application.nativeApplicationVersion ? `Idle ${Application.nativeApplicationVersion}` : null,
