...
import { FILE_LOAD_LIMITS } from './fileLoadPolicy';
...
export async function getGitStatusFiles(sessionId: string): Promise<GitStatusFiles | null> {
    try {
        ...
        if (statusResult.stdout.length > FILE_LOAD_LIMITS.gitStatus.maxOutputCharacters) {
            return null;
        }
        ...
        const diffOutput = diffStatResult.success
            && diffStatResult.stdout.length <= FILE_LOAD_LIMITS.gitStatus.maxOutputCharacters
        ...
        console.error('Failed to fetch git status files');
        return null;
    }
}
...
function parseGitStatusFilesV2(
    ...
    let remainingFiles: number = FILE_LOAD_LIMITS.gitStatus.maxFiles;
    ...
    for (const file of statusSummary.files) {
        ...
        if (remainingFiles > 0 && file.working_dir !== ' ' && file.working_dir !== '.') {
    ...
    for (const untrackedPath of statusSummary.not_added) {
        ...
        if (isDirectory) {
            console.warn('Git status contained an unexpected directory entry');
            continue;
        }
