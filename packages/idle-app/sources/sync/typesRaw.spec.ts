...
describe('Zod Transform - WOLOG Content Normalization', () => {
    ...
    describe('Integration: Complete message flow scenarios', () => {
        ...
        it('accepts the Claude SDK cache TTL usage shape without dropping its breakdown', () => {
            const message = {
                role: 'agent',
                content: {
                    type: 'output',
            ...
            if (result.success && result.data.content.type === 'output') {
                ...
                if (data.type === 'assistant') {
                    expect(data.message.usage?.cache_creation).toEqual({
                        ephemeral_1h_input_tokens: 10,
                        ephemeral_5m_input_tokens: 20,
                    });
                }
        ...
        it('rejects inconsistent and unbounded Claude SDK cache TTL usage', () => {
            const usage = {
                input_tokens: 1_000,
                output_tokens: 50,
                cache_creation_input_tokens: 30,
            ...
            const message = {
                ...
                content: {
                    ...
                    data: {
                        ...
                        uuid: 'assistant-invalid-cache-usage',
                    },
                },
            };

            expect(RawRecordSchema.safeParse(message).success).toBe(false);
