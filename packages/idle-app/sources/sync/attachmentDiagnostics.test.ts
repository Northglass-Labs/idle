...
describe('sanitizeAttachmentUrlHost', () => {
    it('keeps only the host and port from absolute URLs', () => {
        expect(sanitizeAttachmentUrlHost('https://files.example.test/idle/sessions/ref?X-Amz-Signature=secret'))
            .toBe('files.example.test');
...
describe('classifyAttachmentTransferTarget', () => {
    it('classifies URLs on the Idle API host as idle-api', () => {
    ...
    it('classifies other valid hosts as external-storage', () => {
        expect(classifyAttachmentTransferTarget(
            'https://files.example.test/idle/abc?policy=secret',
            'https://relay.example.test',
...
describe('attachment diagnostic serialization', () => {
    it('builds a safe diagnostic from a response', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'blob-upload',
            method: 'POST',
            url: 'https://files.example.test/idle/sessions/ref?X-Amz-Signature=secret&policy=secret',
        ...
        expect(serialized).toContain('"host":"files.example.test"');
        expect(serialized).toContain('"platform":"web"');
        expect(serialized).toContain('"client":"web/1.2.3"');
        expect(serialized).not.toContain('/idle/sessions/ref');
    ...
    it('keeps network messages but not source URLs', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'blob-download',
            method: 'GET',
            url: 'https://files.example.test/idle/sessions/ref?AWSAccessKeyId=secret',
            serverUrl: 'https://relay.example.test',
    ...
    it('does not emit dirty URL hosts from raw diagnostics', () => {
        ...
        const safeDiagnostic = createAttachmentDiagnostic({
            leg: 'blob-download',
            url: 'https://files.example.test/idle/ref?X-Amz-Signature=secret',
            serverUrl: 'https://relay.example.test',
        });

        expect(dirtyDiagnostic).not.toHaveProperty('host');
        expect(safeDiagnostic.host).toBe('files.example.test');
    ...
    it('sanitizes unsafe messages and reasons before serialization', () => {
        const diagnostic = createAttachmentDiagnostic({
            ...
            url: 'https://files.example.test/idle/ref?X-Amz-Signature=secret',
            serverUrl: 'https://relay.example.test',
            message: 'Fetch failed for https://files.example.test/idle/ref?X-Amz-Signature=secret&policy=secret using Bearer secret-token',
            reason: 'Read failed from file:///Users/example/Projects/idle/local.bin and /Users/example/Projects/idle/other.bin',
        });

        expect(diagnostic.message).toContain('files.example.test');
        ...
        expect(serialized).not.toContain('/idle/ref');
    ...
    it('redacts non-http absolute URLs in diagnostic text', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'blob-download',
            method: 'GET',
            message: 'Fetch failed for s3://bucket/idle/ref?X-Amz-Signature=secret',
    ...
    it('redacts desktop local paths with spaces in path segments', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'Failed to fetch',
            reason: 'Read failed from /Users/example/My Projects/idle/blob.bin',
        });
    ...
    it('redacts file URI local paths with spaces in path segments', () => {
        const source = 'Read failed from file:///Users/example/My Projects/idle/blob.bin';
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            reason: source,
        });
    ...
    it('redacts forward-slash Windows local paths with spaces in path segments', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            reason: 'Read failed from C:/Users/example/My Projects/idle/blob.bin',
        });
    ...
    it('redacts assignment-prefixed local paths before diagnostic serialization', () => {
        const sources = [
            'path=/Users/example/My Projects/blob.bin',
            'path=/data/user/0/com.happy/cache/blob.bin',
            'path=C:\\Users\\dev\\My Projects\\blob.bin',
            'path=C:/Users/example/My Projects/blob.bin',
        ];
    ...
    it('sanitizes dirty whitelisted fields when formatting diagnostics for logs', () => {
        const rendered = formatAttachmentDiagnosticForLog({
            leg: 'blob-download',
            method: 'GET',
            host: 'https://files.example.test/idle/ref?X-Amz-Signature=secret',
            target: 'external-storage',
            status: 403,
            statusText: 'Forbidden from https://files.example.test/idle/ref?X-Amz-Signature=secret',
            message: 'Failed to fetch',
            reason: 'missing-blob-key',
        }, {
        ...
        expect(rendered.host).toBe('files.example.test');
    ...
    it('drops dirty non-host values from host output', () => {
        const rendered = [
            ...
            'file:///Users/example/My Projects/idle/blob.bin',
    ...
    it('validates host values extracted from URLs before log formatting', () => {
        ...
        const userInfoHost = formatAttachmentDiagnosticForLog({
            leg: 'blob-download',
            host: ['https://user', ':secret@files.example.test/path'].join(''),
        });

        expect(dirtyKeyValueHost).not.toHaveProperty('host');
        expect(userInfoHost.host).toBe('files.example.test');
    ...
    it('redacts leading-slash attachment refs in messages and reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'missing /idle/session-1/ref?X-Amz-Signature=secret',
            reason: 'ref=/idle/session-1/ref',
        });

        const serialized = JSON.stringify(diagnostic);
        expect(serialized).toContain('[attachment-ref]');
        expect(serialized).not.toContain('/idle/session-1/ref');
        expect(serialized).not.toContain('ref=/idle/session-1/ref');
        expect(serialized).not.toContain('X-Amz-Signature');
    });
...
describe('AttachmentDiagnosticError', () => {
    ...
    it('sanitizes unsafe wrapper error messages', () => {
        const error = createAttachmentDiagnosticError(
            'Blob upload failed for https://files.example.test/idle/ref?X-Amz-Signature=secret using Bearer secret-token at /data/user/0/com.happy/cache/blob.bin',
            {
                leg: 'blob-upload',
                method: 'POST',
                url: 'https://files.example.test/idle/ref?X-Amz-Signature=secret',
        ...
        expect(error.message).toContain('files.example.test');
