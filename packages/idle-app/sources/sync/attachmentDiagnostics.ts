...
export type AttachmentTransferTarget = 'idle-api' | 'external-storage' | 'unknown';
...
const ATTACHMENT_DIAGNOSTIC_ERROR_BRAND: unique symbol = Symbol('idle.attachment-diagnostic-error');
...
export function classifyAttachmentTransferTarget(
    ...
    return host === serverHost ? 'idle-api' : 'external-storage';
}
...
function sanitizeDiagnosticText(value: string | undefined): string | undefined {
    ...
    return value
        ...
        .replace(/(^|[\s(["'])\/?(?:happy|idle|sessions)\/[A-Za-z0-9._~@-]+(?:\/[A-Za-z0-9._~@-]+)+(?:\?[^\s"'<>]*)?/gi, '$1[attachment-ref]');
}
