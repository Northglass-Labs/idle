...
import { readBoundedJsonResponse, readBoundedResponseBytes } from './boundedJsonResponse';
import { getServerUrl } from './serverConfig';
import { streamingFetch } from './streamingFetch';
import { appendFormFile } from './uploadFormFile';

const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB
const MAX_ATTACHMENT_CONTROL_RESPONSE_BYTES = 64 * 1024;

const TransferUrlSchema = z.string().min(1).max(8_192).superRefine((value, context) => {
    try {
        const parsed = new URL(value);
        if ((parsed.protocol !== 'http:' && parsed.protocol !== 'https:')
            || parsed.username !== ''
            || parsed.password !== '') {
            context.addIssue({ code: z.ZodIssueCode.custom, message: 'Invalid transfer URL' });
        }
    } catch {
        context.addIssue({ code: z.ZodIssueCode.custom, message: 'Invalid transfer URL' });
    }
...
const FormFieldsSchema = z.record(
    z.string().min(1).max(256),
    z.string().max(8_192),
).superRefine((fields, context) => {
    if (Object.keys(fields).length > 32) {
        context.addIssue({ code: z.ZodIssueCode.custom, message: 'Too many form fields' });
    }
});

const RequestUploadResultSchema = z.object({
    ref: z.string().min(1).max(2_048),
    uploadUrl: TransferUrlSchema,
    method: z.enum(['PUT', 'POST']),
    formFields: FormFieldsSchema.optional(),
}).strict();

const RequestDownloadResultSchema = z.object({
    downloadUrl: TransferUrlSchema,
}).strict();
...
function validateAttachmentTransferUrl(
    ...
    const parsed = new URL(TransferUrlSchema.parse(value));
...
function isSameOrigin(candidateUrl: string, serverUrl: string): boolean {
    try {
...
export async function requestAttachmentUpload(
    ...
    const requestUrl = `${API_ENDPOINT}/v1/sessions/${encodedSessionId}/attachments/request-upload`;

    let response: Response;
    try {
        response = await streamingFetch(requestUrl, {
            method: 'POST',
            redirect: 'error',
            headers: {
    ...
    try {
        const parsed = RequestUploadResultSchema.parse(
            await readBoundedJsonResponse(response, MAX_ATTACHMENT_CONTROL_RESPONSE_BYTES),
        );
        if (parsed.method === 'POST' && !parsed.formFields) {
            throw new Error('Attachment transfer fields are missing');
        }
        const rewrittenUploadUrl = rewriteLoopbackHost(parsed.uploadUrl);
        result = {
            ...parsed,
            uploadUrl: validateAttachmentTransferUrl(
                rewrittenUploadUrl,
                API_ENDPOINT,
                parsed.method === 'POST' ? 'upload-post' : 'upload-put',
            ),
        };
    } catch (err) {
...
export async function uploadEncryptedBlob(
    ...
    if (upload.method === 'POST') {
        const uploadUrl = validateAttachmentTransferUrl(upload.uploadUrl, serverUrl, 'upload-post');
        const formFields = FormFieldsSchema.parse(upload.formFields);
        ...
        try {
            ...
            response = await streamingFetch(uploadUrl, {
                method: 'POST',
                redirect: 'error',
                body: formData,
            });
    ...
    const uploadUrl = validateAttachmentTransferUrl(upload.uploadUrl, serverUrl, 'upload-put');
...
export async function downloadEncryptedAttachment(
    ...
    const API_ENDPOINT = getServerUrl();
    const encodedSessionId = encodeURIComponent(sessionId);
    const requestUrl = `${API_ENDPOINT}/v1/sessions/${encodedSessionId}/attachments/request-download`;

    let requestRes: Response;
    try {
        requestRes = await streamingFetch(requestUrl, {
            method: 'POST',
            redirect: 'error',
            headers: {
                'Authorization': `Bearer ${credentials.token}`,
    ...
    try {
        const { downloadUrl: rawDownloadUrl } = RequestDownloadResultSchema.parse(
            await readBoundedJsonResponse(requestRes, MAX_ATTACHMENT_CONTROL_RESPONSE_BYTES),
        );
        downloadUrl = validateAttachmentTransferUrl(
            rewriteLoopbackHost(rawDownloadUrl),
            API_ENDPOINT,
            'download',
        );
    } catch (err) {
    ...
    const isServerUrl = isSameOrigin(downloadUrl, API_ENDPOINT);
    const headers: Record<string, string> = {};
    if (isServerUrl) {
    ...
    try {
        blobRes = await streamingFetch(downloadUrl, { headers, redirect: 'error' });
    } catch (err) {
        const message = errorMessageFromUnknown(err);
    ...
    try {
        buffer = await readBoundedResponseBytes(blobRes, MAX_FILE_SIZE);
    } catch (err) {
        const message = errorMessageFromUnknown(err);
