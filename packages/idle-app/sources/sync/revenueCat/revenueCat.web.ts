...
class RevenueCatWeb implements RevenueCatInterface {
    ...
    setLogLevel(level: LogLevel): void {
        ...
        console.log('RevenueCat log-level configuration is unavailable on web');
    }
    ...
    async presentPaywall(options?: PaywallOptions): Promise<PaywallResult> {
        try {
            ...
            console.error('Failed to present paywall on web');
        ...
        const transformPackages = (packages: Package[]) => {
            return packages
                .map(pkg => {
                    ...
                    if (!product) {
                        console.error('Paywall package has no product');
                        return null;
                    }
                    return {
