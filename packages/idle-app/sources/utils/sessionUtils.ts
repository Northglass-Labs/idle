...
} from '@/sync/sessionOperationalState';
...
/**
 * Generates a deterministic avatar ID per session.
 *
 * The seed includes machine, path, and session identity so parallel sessions
 * in one project remain distinct and deterministic across reconnects.
 */
 ...
 * Uses flavor-specific commands which work without idle-agent auth.
 */
export function getResumeCommand(session: Session): string | null {
    ...
    return metadata ? buildResumeCommand(metadata) : null;
}

export function getResumeCommandBlock(session: Session): ResumeCommandBlock | null {
    const metadata = getOperationalSessionMetadata(session.metadata);
    return metadata ? buildResumeCommandBlock(metadata) : null;
}
...
export function formatLastSeen(activeAt: number, isActive: boolean = false): string {
    ...
    if (diffSeconds < 60) {
        ...
        // > 24h: show the device-local date AND time, never "N days ago".
        // Users wanted concrete date+time instead of relative phrases — a
        // session last touched yesterday says "May 11 3:42 PM", not "1 day
        // ago" and not just "May 11" (which left users hunting for when).
