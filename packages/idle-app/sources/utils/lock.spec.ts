...
describe('AsyncLock', () => {
    ...
    it('serializes concurrent inLock callers — each runs to completion before next starts', async () => {
        ...
        // their work must NOT interleave. We track entry+exit timestamps and assert no overlap.
        const lock = new AsyncLock();
        const log: string[] = [];
        ...
        const work = (label: string) =>
            lock.inLock(async () => {
                ...
                // Yield to event loop so an unprotected version would interleave here.
    ...
    it('preserves FIFO order — callers run in the order they entered the lock', async () => {
        ...
        // Kick off A first; it acquires immediately. Then B + C queue behind it.
        ...
        const b = lock.inLock(async () => {
            order.push('B');
        });
        const c = lock.inLock(async () => {
            order.push('C');
        });
    ...
    it('releases the lock even when the body throws', async () => {
        ...
        // First caller throws. If unlock() is missed, the second caller deadlocks forever.
        await expect(
            lock.inLock(async () => {
        ...
        // Subsequent caller must still acquire — proves unlock() ran in the finally.
    ...
    it('handles many queued callers without losing any', async () => {
        ...
        expect(new Set(order).size).toBe(N); // every caller ran
