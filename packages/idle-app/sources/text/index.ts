...
if (settings.settings.preferredLanguage && settings.settings.preferredLanguage in translations) {
    ...
    found = true;
    console.log('Using preferred language');
}

// Read from device
if (!found) {
    let locales = Localization.getLocales();
    console.log('Reading device locales');
    for (let l of locales) {
        if (l.languageCode) {
            // Chinese locale selection uses the platform-provided script code.
            if (l.languageCode === 'zh') {
                let chineseVariant: string | null = null;

                // Prefer a supported script-specific translation when one is available.
                ...
                console.log('Selected Chinese script variant');

                if (chineseVariant && chineseVariant in translations) {
                    currentLanguage = chineseVariant as SupportedLanguage;
                    console.log('Using Chinese language variant');
                    break;
                }

                currentLanguage = 'zh-Hans';
                console.log('Using simplified Chinese fallback');
                break;
            }
            ...
            if (l.languageCode in translations) {
                ...
                console.log('Using supported device locale');
                break;
            }
        }
    }
...
export function t<K extends TranslationKey>(
    ...
    try {
        ...
        console.warn('Invalid translation value type');
        return key;
    } catch (error) {
        console.error('Translation evaluation failed');
        return key;
    }
}
