...
async function readFileContent(sessionId: string, filePath: string): Promise<string | null> {
    ...
    return decoded && !decoded.isBinary ? decoded.text : null;
}
...
export const FileViewPanel = React.memo(function FileViewPanel({
    ...
    React.useEffect(() => {
        ...
        (async () => {
            try {
                const fileResponse = await sessionReadFile(
                    sessionId,
                    filePath,
                    FILE_LOAD_LIMITS.explicitOpen.maxBytesPerResponse,
                );
                ...
                const hash = await computeSHA256(decoded.text);
                setFileState({ kind: 'loaded', content: decoded.text, originalHash: hash });
                setEditContent(decoded.text);
            } catch {
                if (!cancelled) {
