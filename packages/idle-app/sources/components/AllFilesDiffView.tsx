...
/**
 * Loads a bounded set of diffs with limited concurrency, then renders them in a single ScrollView.
...
export const AllFilesDiffView = React.memo(function AllFilesDiffView({
    ...
    // Deduplicate and cap the rendered inventory before allocating diff work.
    // Preserve an explicitly requested file even if it falls beyond the cap.
    const files = React.useMemo(() => {
        if (!gitStatusFiles) return [];
        const selected: GitFileStatus[] = [];
        ...
        collectFiles: for (const group of [gitStatusFiles.stagedFiles, gitStatusFiles.unstagedFiles]) {
            for (const file of group) {
                ...
                selected.push(file);
                if (selected.length >= FILE_LOAD_LIMITS.allDiff.maxFiles) {
                    break collectFiles;
                }
            }
        }

        if (scrollToFile && !seen.has(scrollToFile) && selected.length > 0) {
            const preferred = gitStatusFiles.stagedFiles.find((file) => file.fullPath === scrollToFile)
                ?? gitStatusFiles.unstagedFiles.find((file) => file.fullPath === scrollToFile);
            if (preferred) {
    ...
    React.useEffect(() => {
        ...
        const sessionPath = getOperationalSessionMetadata(session?.metadata)?.path ?? null;
        ...
        (async () => {
            const fetched = await mapWithConcurrency(
                toFetch,
                FILE_LOAD_LIMITS.allDiff.maxFiles,
                FILE_LOAD_LIMITS.allDiff.concurrency,
                async (file): Promise<FileDiffResult> => {
                    if (!sessionPath) {
                    ...
                    try {
                        if (file.status === 'untracked') {
                            ...
                            const res = await sessionReadFile(
                                ...
                                FILE_LOAD_LIMITS.allDiff.maxBytesPerResponse,
                            );
                            ...
                            if (!decoded || decoded.isBinary) {
                                return { file, content: null, error: 'Failed to decode file' };
                            }
                            return { file, content: { kind: 'newFile', contents: decoded.text }, error: null };
                        }

                        const res = await sessionGitDiff(sessionId, {
                            path: gitDiffPath,
                            mode: 'head',
                            timeout: 5000,
                            maxBytes: FILE_LOAD_LIMITS.allDiff.maxBytesPerResponse,
                        });
                        if (!res.success) {
