...
export async function getCommandSuggestions(sessionId: string, query: string): Promise<{
    ...
    try {
        // Use the command search cache with enough results for the full command set.
        ...
        console.error('Failed to fetch command suggestions');
        return [];
    }
}
...
export async function getFileMentionSuggestions(sessionId: string, query: string): Promise<{
    ...
    try {
        // Use the file search cache with enough results for useful file mentions.
        ...
        console.error('Failed to fetch file suggestions');
        return [];
    }
}
