...
import { useSettingMutable, useSetting, useAllMachines, useSession } from '@/sync/storage';
import { ActiveSessionsGroup } from './ActiveSessionsGroup';
import { hapticsLight } from './haptics';
import { notifySessionActionsTriggered } from './sessionActionsHintBus';
import { SessionActionsHint } from './SessionActionsHint';
import { loadSessionActionsHintFlag, markSessionActionsHintSeen } from '@/sync/persistence';
import { shouldShowSessionActionsHint } from './labOnboardingPersist';
import { t } from '@/text';
...
export function SessionsList() {
    ...
    // Users can choose compact or detailed active-session rows.
    const compactSessionView = useSetting('compactSessionView');
    const machines = useAllMachines();

    // First-launch ⋯ coachmark. Compute once on mount
    // from the mmkv flag + the snapshotted "has paired terminal + 2+ sessions"
    // gate. Bus dismissal happens via SessionActionsHint's effect when any
    // ⋯ tap or long-press fires. Auto-dismisses after 8s.
    const sessionCount = React.useMemo(
        () => (data ?? []).filter((item) => item.type === 'session').length,
        [data],
    );
    const [actionsHintVisible, setActionsHintVisible] = React.useState(() =>
        shouldShowSessionActionsHint(loadSessionActionsHintFlag(), machines.length > 0, sessionCount),
    );
    React.useEffect(() => {
        if (actionsHintVisible) return;
        if (shouldShowSessionActionsHint(loadSessionActionsHintFlag(), machines.length > 0, sessionCount)) {
            setActionsHintVisible(true);
        }
    }, [actionsHintVisible, machines.length, sessionCount]);
    const handleActionsHintDismiss = React.useCallback(() => {
        markSessionActionsHintSeen();
        setActionsHintVisible(false);
    }, []);
    ...
    // the deselected and selected rows re-render, instead of the
    ...
    const renderItem = React.useCallback(({ item, index }: { item: SessionListViewItem, index: number }) => {
        switch (item.type) {
            ...
            case 'active-sessions': {
                // Select the compact or detailed active-session renderer.
    ...
    }, [selectedSessionId, data, toggleArchived, compactSessionView]);
    ...
    const HeaderComponent = React.useCallback(() => {
        return (
            <>
                <UpdateBanner />
                <SessionActionsHint
                    visible={actionsHintVisible}
                    onDismiss={handleActionsHintDismiss}
                />
            </>
        );
    }, [actionsHintVisible, handleActionsHintDismiss]);
...
const SessionItem = React.memo(({ session, selected, isFirst, isLast, isSingle }: {
    ...
    // Full session object for the actions popover (rename / groups / archive) —
    // the row itself renders from the lightweight SessionRowData projection.
    ...
    // Native long-press surfaces the same SessionActionsPopover that web already
    // shows on right-click (bottom sheet on native; includes Idle's group
    // actions, which the plain action alert lacks).
    ...
    return (
        ...
        {/* Popover renders on every platform: web shows as positioned menu at
            the right-click point; native shows as a bottom sheet (see
            SessionActionsPopover.nativeContainer / nativeSheet styles). */}
        {fullSession && (
