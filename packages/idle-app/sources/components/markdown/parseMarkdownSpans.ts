...
// Allowed URL protocols — everything else is blocked to prevent XSS
const SAFE_URL_PATTERN = /^(https?:\/\/|mailto:|tel:|\/|\.\.?\/|#)/i;

/**
 * Security: only allow safe URL protocols. Blocks javascript:, data:,
 * vbscript:, and any other unknown protocol scheme.
 */
function sanitizeUrl(url: string): string | null {
    const trimmed = url.trim();
    if (!trimmed) return null;
    if (SAFE_URL_PATTERN.test(trimmed)) return url;
    // Relative paths without protocol (e.g. "readme.md") are safe
...
export function parseMarkdownSpans(markdown: string, header: boolean) {
    ...
    while ((match = pattern.exec(markdown)) !== null) {
        ...
        if (match[1]) {
            ...
            if (match[7]) {
                const safeUrl = sanitizeUrl(match[7]);
                spans.push({ styles: [], text: match[6], url: safeUrl });
            } else {
                // If no URL part, treat as plain text (strip brackets)
