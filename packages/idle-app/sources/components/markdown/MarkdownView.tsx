...
export const MarkdownView = React.memo((props: {
    ...
    // Mobile long-press selection owns the copy gesture when markdownCopyV2 is enabled;
    // nested selectable spans would otherwise open the native copy modal concurrently.
    ...
    const handleLinkPress = React.useCallback((url: string) => {
        ...
        if (Platform.OS === 'web') {
            // openExternalUrl handles both browser and desktop-shell openers.
            void openExternalUrl(url);
            return;
        }

        // Native: honor the user's linksOpenIn preference (in-app vs external).
        void openLink(url);
    }, []);
    ...
    const handleLongPress = React.useCallback(() => {
        try {
            ...
            console.error('Failed to store text selection');
...
function RenderImageBlock(props: { url: string, alt: string, first: boolean, last: boolean }) {
    const policy = React.useMemo(() => classifyMarkdownImage(props.url), [props.url]);
    const [remoteLoadGranted, setRemoteLoadGranted] = React.useState(false);
    const renderUri = getMarkdownImageRenderUri(policy, remoteLoadGranted);
    const accessibleLabel = props.alt || t('markdown.imageAlt');

    React.useEffect(() => {
        setRemoteLoadGranted(false);
    }, [props.url]);
            ...
            <View style={[style.imageBlock, props.first && style.first, props.last && style.last]}>
                {policy.kind === 'remote' ? (
                    <Pressable
                        ...
                        accessibilityLabel={t('markdown.loadRemoteImage', { host: policy.hostname })}
                        onPress={() => setRemoteLoadGranted(true)}
                        style={style.imageConsent}
                    >
                        <Text style={style.imageConsentTitle}>{accessibleLabel}</Text>
                        <Text style={style.imageConsentBody}>{t('markdown.remoteImageBlocked')}</Text>
                        <Text style={style.imageConsentAction}>{t('markdown.loadRemoteImage', { host: policy.hostname })}</Text>
                    </Pressable>
                ) : (
                    <View style={style.imageConsent} accessibilityLabel={t('markdown.blockedImage')}>
                        <Text style={style.imageConsentTitle}>{accessibleLabel}</Text>
                        <Text style={style.imageConsentBody}>{t('markdown.blockedImage')}</Text>
                    </View>
                )}
            </View>
        );
