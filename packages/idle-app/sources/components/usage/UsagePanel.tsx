...
export const UsagePanel: React.FC<{ sessionId?: string }> = ({ sessionId }) => {
    ...
    const topTokenTypes = Object.entries(totals.tokensByType)
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            {/* Token breakdown */}
            {topTokenTypes.length > 0 && (
                <ItemGroup title={t('usage.byType')}>
                    <View style={{ padding: 16 }}>
                        {topTokenTypes.map(([type, tokens]) => (
                            <UsageBar
                                key={type}
                                label={formatUsageTypeLabel(type)}
                                value={tokens}
...
function formatUsageTypeLabel(type: string): string {
    return ({
        input: 'Input',
        output: 'Output',
