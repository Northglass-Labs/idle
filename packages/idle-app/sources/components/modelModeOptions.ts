...
export function getClaudeEffortLevels(): EffortLevel[] {
    // Mirrors Claude Code's --effort flag: low, medium, high, xhigh, max.
    // Passed through to the API's output_config.effort parameter on each turn.
    return [
        { key: 'low', name: 'low' },
...
export function getCodexEffortLevels(): EffortLevel[] {
    // Mirrors Codex's ReasoningEffort: none, minimal, low, medium, high, xhigh.
    // We expose low-xhigh to match the Claude picker shape; none/minimal are
    // edge cases the CLI passes through if explicitly typed via /think slash.
    return [
        { key: 'low', name: 'low' },
        { key: 'medium', name: 'medium' },
        { key: 'high', name: 'high' },
...
export function getEffortLevelsForModel(flavor: AgentFlavor, _modelKey: string): EffortLevel[] {
    // Effort is provider-wide rather than model-pinning-specific, so sessions
    // inheriting the default model retain the same effort controls.
    if (flavor === 'claude') {
