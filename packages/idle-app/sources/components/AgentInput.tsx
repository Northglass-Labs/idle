...
import { getOperationalSessionMetadata, useSetting } from '@/sync/storage';
import { hackMode, hackModes } from '@/sync/modeHacks';
import { Theme } from '@/theme';
import { t } from '@/text';
import { Metadata } from '@/sync/storageTypes';
import { getEnsoContextSize } from '@/utils/contextWindow';
import { isSandboxActive } from '@/utils/sandboxClassification';
import { ContextEnso } from './ContextEnso';
import { ContextEnsoSheet } from './ContextEnsoSheet';
import { ConnectionStatusDetailSheet } from './ConnectionStatusDetailSheet';
...
type StatusRowProps = {
    ...
    enso: { usedTokens: number | null; totalTokens: number; onPress: () => void } | null;
    onConnectionStatusPress?: () => void;
...
const AgentInputStatusRow = React.memo(function AgentInputStatusRow(p: StatusRowProps) {
    ...
    const showPermissionBadge = !!p.displayPermissionMode
        ...
        && !p.zenMode
        && !!p.permissionLabel;
    if (!p.connectionStatus && !p.enso && !showPermissionBadge) {
        return null;
    }
    return (
        <View style={{
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, gap: 11 }}>
                {p.connectionStatus && (
                    <>
                        <Pressable
                            onPress={p.onConnectionStatusPress}
                            hitSlop={8}
                            accessibilityRole="button"
                            accessibilityLabel="Open connection status details"
                            testID="connection-status-pressable"
                ...
                {p.enso && (
                    <View style={{
                        ...
                        <ContextEnso
                            usedTokens={p.enso.usedTokens}
                            totalTokens={p.enso.totalTokens}
                            size={14}
                            onPress={p.enso.onPress}
                            testID="context-enso"
                        />
                    </View>
                )}
            ...
            {showPermissionBadge && (() => {
                const permColor = p.isSandboxedYoloMode ? theme.colors.permission.sandboxedYolo :
        ...
        <View style={{
            ...
            {p.machineName !== undefined && p.onMachineClick && (
                <Pressable
                    ...
                    accessibilityRole="button"
                    accessibilityLabel={`View machine: ${p.machineName ?? 'No machines'}`}
                    style={(s) => ({
                        flexDirection: 'row',
                        alignItems: 'center',
                        borderRadius: Platform.select({ default: 16, android: 20 }),
            ...
            {p.currentPath && p.onPathClick && (
                <Pressable
                    ...
                    accessibilityRole="button"
                    accessibilityLabel={`View project path: ${p.currentPath}`}
                    hitSlop={{ top: 5, bottom: 10, left: 0, right: 0 }}
                    style={(s) => ({
                        flexDirection: 'row',
                        alignItems: 'center',
...
export const AgentInput = React.memo(React.forwardRef<MultiTextInputHandle, AgentInputProps>((props, ref) => {
    ...
    const operationalMetadata = getOperationalSessionMetadata(props.metadata);
    ...
    const isCodex = operationalMetadata?.flavor === 'codex' || props.agentType === 'codex';
    const isGemini = operationalMetadata?.flavor === 'gemini' || props.agentType === 'gemini';
    const isOpenClaw = operationalMetadata?.flavor === 'openclaw' || props.agentType === 'openclaw';
    ...
    // metadata.sandbox is the SECURITY CLAIM the CLI makes about the spawned
    // process. Only an object with explicit runtime-enforcement evidence may
    // { enabled: true } counts — empty / malformed / truthy-non-bool payloads
    // collapse to false. Test coverage: sources/utils/sandboxClassification.spec.ts.
    const isSandboxEnabled = React.useMemo(
        () => isSandboxActive(operationalMetadata?.sandbox),
    ...
    // Context indicator: replace the text "<N>% left" with the
    // ContextEnso ring (Northglass brand symbol as functional UI).
    // We pass `null` (not 0) for usedTokens when there's no usage data, so
    // ContextEnso renders the empty stroke rather than a zero-fill arc.
    const rawContextSize = props.usageData?.contextSize;
    const alwaysShow = props.alwaysShowContextSize ?? false;
    const hasUsage = typeof rawContextSize === 'number' && rawContextSize > 0;
    const shouldShowEnso = hasUsage || alwaysShow;
    const ensoUsedTokens = hasUsage ? rawContextSize! : null;
    const ensoTotalTokens = getEnsoContextSize(operationalMetadata);
    const modelLabelForEnso = operationalMetadata?.currentModelCode ?? null;
    // Sheet visibility (Idle): ContextEnsoSheet detail view + UI behavior
    // connection-status detail sheet.
    const [ensoSheetVisible, setEnsoSheetVisible] = React.useState(false);
    const onEnsoPress = React.useCallback(() => setEnsoSheetVisible(true), []);
    const onEnsoSheetClose = React.useCallback(() => setEnsoSheetVisible(false), []);
    const [connectionSheetVisible, setConnectionSheetVisible] = React.useState(false);
    const onConnectionStatusPress = React.useCallback(() => setConnectionSheetVisible(true), []);
    const onConnectionSheetClose = React.useCallback(() => setConnectionSheetVisible(false), []);
    ...
    const handleAbortPress = React.useCallback(async () => {
        ...
        try {
            ...
            console.error('Abort RPC call failed');
        } finally {
            setIsAborting(false);
        }
    }, [props.onAbort]);
        ...
        <View style={[
            ...
            <View style={[
                ...
                <AgentInputStatusRow
                    ...
                    enso={shouldShowEnso ? { usedTokens: ensoUsedTokens, totalTokens: ensoTotalTokens, onPress: onEnsoPress } : null}
                    onConnectionStatusPress={onConnectionStatusPress}
            ...
            <ContextEnsoSheet
                visible={ensoSheetVisible}
                onClose={onEnsoSheetClose}
                usedTokens={ensoUsedTokens}
                totalTokens={ensoTotalTokens}
                model={modelLabelForEnso}
            />
            <ConnectionStatusDetailSheet
                visible={connectionSheetVisible}
                onClose={onConnectionSheetClose}
            />
        </View>
    );
}));
