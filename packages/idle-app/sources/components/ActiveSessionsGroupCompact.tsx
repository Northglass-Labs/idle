...
import { useAllMachines, useSessionGitStatus, useLocalSetting, useSession } from '@/sync/storage';
...
const CompactSessionRow = React.memo(({ session, selected, showBorder }: { session: SessionRowData; selected?: boolean; showBorder?: boolean }) => {
    ...
    const status = session.hasUnread
        ...
        : baseStatus;
    // Idle: local rename override (customSessionNames) applies on top of the
    // store-computed row name.
    const customNames = useLocalSetting('customSessionNames');
    const nameOverride = customNames?.[session.id];
    const sessionName = nameOverride && nameOverride.trim() ? nameOverride : session.name;
    ...
    // Swipeable wrapper removed; archive
    // now lives under ⋯. One-time leftward-swipe detection fires a toast that
    // tells users where archive went.
