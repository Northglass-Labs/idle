...
import { createItemExpandState, itemShouldExpand } from './itemExpandState';
...
export interface ItemProps {
    ...
    // Long-form description that wraps freely and appears below the row
    // when expanded via tap. If absent, the row behaves identically to before.
    // Tapping the row body toggles expansion when no onPress / onLongPress / copy
    // is provided (otherwise the existing handlers take precedence and longDescription
    // is rendered always-visible as a multiline subtitle).
    longDescription?: string;
    // Optional: render longDescription expanded on first paint (e.g., for a
    // "currently selected" row). Defaults to false.
    defaultExpanded?: boolean;
    // UI behavior: optional helper-text shown via a tappable ⓘ button on the right.
    // Independent of the row's own onPress so navigation rows can still
    // surface a full description without burying it. Tap ⓘ → Modal.alert
    // with title + info body. Matches iOS Settings convention.
    info?: string;
}
...
export const Item = React.memo<ItemProps>((props) => {
    ...
    // Item expand state. Only used when longDescription is provided AND
    // no other handler is set on the row (tap-to-toggle uses the row's own onPress slot).
    const [expandState, setExpandState] = React.useState(() => createItemExpandState({ defaultExpanded }));
    const expanded = itemShouldExpand(expandState, longDescription);
    const toggleExpand = React.useCallback(() => {
        setExpandState((prev) => ({ expanded: !prev.expanded }));
    }, []);
    ...
    // When longDescription is provided AND no onPress/copy handler exists,
    // the row's tap toggles the expanded panel.
    const handlePress = onPress ?? (longDescription && !copy ? toggleExpand : undefined);

    // UI behavior — ⓘ button handler. Independent of the row's own press handlers so
    // navigation rows can still expose helper text. Modal.alert is the right
    // primitive for short-to-medium descriptions; if a row needs a richer
    // explanation, it should use longDescription instead.
    const handleInfoPress = React.useCallback(() => {
        if (info) {
            Modal.alert(title, info);
        }
    }, [info, title]);
        ...
        <>
            <View style={[styles.container, containerPadding, style]}>
                ...
                <View style={styles.rightSection}>
                    ...
                    {info && !loading && (
                        // UI behavior: ⓘ helper button. Pressable with hitSlop for fingertip
                        // ergonomics; lives inside the outer Pressable but its onPress
                        // doesn't fire the outer one (React Native handles this via
                        // gesture responder).
                        <Pressable
                            ...
                            hitSlop={8}
                            style={({ pressed }) => [
                                { marginLeft: 6, opacity: pressed ? 0.6 : 1 },
                            ]}
                        >
                            <Ionicons
            ...
            {/* Expanded longDescription panel */}
            {expanded && longDescription && (
                <View style={styles.longDescriptionPanel}>
                    <Text style={styles.longDescriptionText}>{longDescription}</Text>
                </View>
            )}
