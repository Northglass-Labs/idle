...
import { getOperationalSessionMetadata, useLocalSetting } from '@/sync/storage';
import { layout } from "./layout";
import { parseLocalCommandMessage, isUserSlashCommandEcho } from './parseLocalCommandMessage';

// Render per-message timestamps next to each user and agent bubble. AgentEventBlock
// uses the same helper for usage-limit resets so every timestamp follows one format.
//
// > 24h rule: when a message is older than 24 hours by device-local time, the
// timestamp prefixes the date (e.g. "May 14, 3:42 PM" instead of just
// "3:42 PM"). Same-day messages stay compact. Prior-year messages add the
// year. Keeps the chat readable when you scroll back through old sessions
// without needing a date separator row.
const formatTime = (timestamp: number): string => {
    try {
        // Timestamps under 1e12 are epoch seconds (e.g. Claude SDK endsAt),
        // those above are epoch milliseconds (e.g. server createdAt from Date.getTime())
...
function UserTextBlock(props: {
  ...
  const canFork = Boolean(props.onForkFromUserMessage)
    && (Boolean(rewindPointId) || operationalMetadata?.flavor === 'codex');
  ...
  const isClaudeFlavor = operationalMetadata !== null
    && (!operationalMetadata.flavor || operationalMetadata.flavor === 'claude');
  ...
  return (
    <View style={styles.userMessageContainer}>
      ...
      {showTimestamps && props.message.createdAt > 0 && (
        <Text style={styles.userTimestamp}>{formatTime(props.message.createdAt)}</Text>
      )}
    </View>
  );
}
...
function AgentTextBlock(props: {
  ...
  return (
    <View>
      ...
      {showTimestamps && props.message.createdAt > 0 && (
        <Text style={styles.agentTimestamp}>{formatTime(props.message.createdAt)}</Text>
      )}
    </View>
  );
}
...
function AgentEventBlock(props: {
  ...
  if (props.event.type === 'limit-reached') {
    // Uses the module-level formatTime helper (defined at top of file).
    return (
      <View style={styles.agentEventContainer}>
