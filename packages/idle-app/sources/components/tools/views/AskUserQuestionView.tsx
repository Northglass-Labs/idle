...
import { parseAskUserQuestionInput } from '../knownTools';
...
export const AskUserQuestionView = React.memo<ToolViewProps>(({ tool, sessionId }) => {
    ...
    const input = parseAskUserQuestionInput(tool.input);

    if (!input) {
        return (
            <ToolSectionView>
                <View style={styles.invalidContainer} accessibilityRole="alert">
                    <Ionicons name="alert-circle-outline" size={18} color={theme.colors.textSecondary} />
                    <Text style={styles.invalidText}>{t('common.error')}</Text>
                </View>
            </ToolSectionView>
        );
    }
    ...
    const handleSubmit = React.useCallback(async () => {
        ...
        // Freeze the captured answers while the request is in flight so later
        // taps cannot diverge from the submitted payload.
        setIsSubmitted(true);
