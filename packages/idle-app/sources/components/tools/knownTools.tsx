...
const ASK_USER_QUESTION_LIMITS = {
    questions: 4,
    optionsPerQuestion: 4,
    questionLength: 2000,
    headerLength: 64,
    optionLabelLength: 200,
    optionDescriptionLength: 1000,
} as const;

const boundedNonBlankString = (maxLength: number) => z.string()
    .min(1)
    .max(maxLength)
...
export const askUserQuestionInputSchema = z.object({
    questions: z.array(z.object({
        question: boundedNonBlankString(ASK_USER_QUESTION_LIMITS.questionLength),
        header: boundedNonBlankString(ASK_USER_QUESTION_LIMITS.headerLength),
        options: z.array(z.object({
            label: boundedNonBlankString(ASK_USER_QUESTION_LIMITS.optionLabelLength),
            description: boundedNonBlankString(ASK_USER_QUESTION_LIMITS.optionDescriptionLength),
        }).strict()).min(1).max(ASK_USER_QUESTION_LIMITS.optionsPerQuestion),
        multiSelect: z.boolean(),
    }).strict()).min(1).max(ASK_USER_QUESTION_LIMITS.questions),
}).strict();
...
export function parseAskUserQuestionInput(input: unknown): AskUserQuestionInput | null {
    ...
    if (
        ...
        || questions.length < 1
        || questions.length > ASK_USER_QUESTION_LIMITS.questions
    ) {
        return null;
    }
    ...
    for (const question of questions) {
        ...
        if (
            ...
            || options.length < 1
            || options.length > ASK_USER_QUESTION_LIMITS.optionsPerQuestion
        ) {
            return null;
        }
    }
...
export const knownTools = {
    ...
    'AskUserQuestion': {
        title: (opts: { metadata: Metadata | null, tool: ToolCall }) => {
            const input = parseAskUserQuestionInput(opts.tool.input);
            return input?.questions[0].header ?? t('tools.names.question');
        },
        icon: ICON_QUESTION,
        ...
        extractSubtitle: (opts: { metadata: Metadata | null, tool: ToolCall }) => {
            ...
            return count === 1
                ...
                : t('tools.askUserQuestion.multipleQuestions', { count });
        }
    },
